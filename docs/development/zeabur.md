...
Web 游戏代理在客户端接受 gzip 时对 JSON 流进行压缩，不依赖 Next 对流式 Route Handler 自动压缩；设置 `Vary: Accept-Encoding`，压缩表示使用弱 ETag，并将条件请求映射回后端校验值。304 保持空响应。轮询失败后从请求结束起按 1、2、4、8、16 秒递增退避（带随机抖动，最多 30 秒，且不短于正常轮询间隔），成功后恢复正常频率，避免超时后仅等 16 毫秒便再次发起请求。
