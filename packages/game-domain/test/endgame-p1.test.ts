...
import {enterGuildRaid,leaveGuildRaid,guildRaidAction,guildRaidView,settleGuildRaid} from '../src/rules/guild-raid.js';
...
function fixture(){const s=createMoltenCoreDemo().state;s.party=s.party.slice(0,4);s.growthPolicy='player';enterGuildRaid(s,'onyxias-lair');return s;}
function start(){const s=fixture();s.guildRaid.clearedPacks=['onyxia-warders'];guildRaidAction(s,{type:'raidStart',bossId:'onyxia'});return s;}
...
test('Onyxia route has real loot and keeps independent weekly progress from MC',()=>{
 let s=fixture();assert.equal(s.party.length,24);assert.deepEqual(raidRoutePlan(s.guildRaid,'onyxia'),['onyxia-warders','onyxia']);assert.equal(raidMapView(s,s.guildRaid,true).name,'奥妮克希亚的巢穴');
 assert.ok(raidLoot.onyxia.length>10);assert.ok(raidLoot.onyxia.every((id:number)=>items[id]));
 assert.ok(raidLoot.onyxia.includes(17075));assert.ok(!raidLoot.onyxia.includes(13139),'boss preview omits the shared world-blue pool');
 s.guildRaid.claims.onyxia=true;s.guildRaid.cleared=['onyxia'];leaveGuildRaid(s);enterGuildRaid(s);assert.equal(s.guildRaid.raidId,'molten-core');assert.deepEqual(s.guildRaid.claims,{});
 s.guildRaid.claims.lucifron=true;leaveGuildRaid(s);s=JSON.parse(JSON.stringify(s));enterGuildRaid(s,'onyxias-lair');assert.equal(s.guildRaid.claims.onyxia,true);assert.deepEqual(s.guildRaid.cleared,['onyxia']);
});
...
test('Onyxia reward settlement is idempotent and the weekly reset is independent',()=>{
 let s=start();for(const e of s.combat.enemies)e.hp=0;s=advance(s,s.wallAt+100).state;const r=s.guildRaid;assert.ok(r.claims.onyxia);const count=r.rewards.length;assert.ok(count>0);settleGuildRaid(s);assert.equal(r.rewards.length,count);leaveGuildRaid(s);s.wallAt+=604800000;enterGuildRaid(s,'onyxias-lair');assert.deepEqual(s.guildRaid.claims,{});
});
test('service can enter, persist and leave an Onyxia instance',async()=>{
 ...
 let view=await service.command(save.id,{type:'enterDungeon',contentId:'onyxias-lair',capacity:25,requestId:'ony-enter'});assert.equal(view.state!.guildRaid.raidId,'onyxias-lair');assert.equal(view.state!.party.length,24);
 const restarted=new GameService(store,{contentVersion:'test',now:()=>Date.UTC(2026,8,23),seed:()=>60325});view=await restarted.command(save.id,{type:'leaveInstance',requestId:'ony-leave'});assert.equal(view.state!.party.length,4);assert.equal(view.state!.guildRaid.active,false);assert.equal(guildRaidView(view.state).name,'奥妮克希亚的巢穴');
});
test('configured 25-player raid completes Onyxia through the real combat engine',()=>{
 ...
 assert.equal(s.combat,null);assert.equal(s.guildRaid.attempts.at(-1)?.won,true,JSON.stringify({attempt:s.guildRaid.attempts.at(-1),phase:s.lastCombat?.raidEncounter?.phase,boss:s.lastCombat?.enemies[0]?.hp,events:s.lastCombat?.raidEncounter?.events?.slice(-5)}));assert.ok(s.guildRaid.claims.onyxia);
});
