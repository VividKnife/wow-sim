...
test('command mode survives server restart and local pause checkpoints with no timer gain',async()=>{
 ...
 assert.equal(persisted.clock,claimed.state.clock);assert.equal(persisted.combat.pull.startsAt,claimed.state.combat.pull.startsAt);
 const credentials={localClientId:base.clientId,localSessionId:saved.session.id};
 const enemy=persisted.combat.enemies[0];
 const marked=await f.send({type:'combatCommand',order:'mark',encounterId:persisted.combat.id,targetId:enemy.id,mark:'skull',...credentials});
