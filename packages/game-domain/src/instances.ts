...
const instanceCommands = new Set(['strategy', 'settings', 'petCommand', 'cast', 'useItem', 'rest', 'stop', 'abandonCombat', 'revive', 'resurrect', 'reincarnate', 'soulstoneRevive', 'dungeonNext','dungeonNavigate','dungeonPause', 'dungeonInteract', 'dungeonSkip', 'equip', 'equipBag', 'sortBag', 'discardJunk', 'lockItem', 'applyEnchant', 'useBandage', 'disenchant', 'disenchantAll', 'loot', 'conjure', 'talent']);
...
export async function startInstance(this: GameService, tx: Transaction, c: Character, cmd: Rules, now: number) {
    ...
    instance.sequence++;
    instance.nextEventAt = now + simulationInterval(s.combat, (await account(tx, c.accountId)).lastSeenAt, now);
...
export async function advanceInstance(this: GameService, instanceId: string, workerId: string, epoch: number, now = this.now()) {
    return this.store.transaction(async tx => {
        ...
        if (instance.status !== 'running' || instance.nextEventAt > now) return false;
        requireThat(instance.contentVersion === this.contentVersion, 'CONTENT_VERSION', '副本内容版本暂不可用');
        let lastSeenAt = 0;
        ...
        instance.sequence++;
        await this.persistInstance(tx, instance, now, key);
