...
import {log,rng,spellInfo,knownRank} from './character.js';
...
export const moltenCoreBosses = [
 {id:'lucifron',entry:12118,name:'鲁西弗隆',subtitle:'诅咒与末日',hp:180000,low:950,high:1300,
  description:'副坦接住两名护卫，优先击杀护卫。法师解除诅咒，牧师驱散即将引爆的末日。',reward:'净化符文'},
 {id:'magmadar',entry:11982,name:'玛格曼达',subtitle:'狂暴与熔岩',hp:230000,low:1300,high:1700,
  description:'猎人用宁神射击平息狂暴，矮人牧师保护主坦免受恐惧，团队离开熔岩炸弹的落点。',reward:'熔火战功'},
];
...
export function moltenCoreTick(s,actors,hurt) {
 const raid=s.combat?.raidEncounter;if(!raid)return;
 const boss=s.combat.enemies.find(e=>e.id===raid.bossId),living=actors.filter(c=>c.hp>0);
 if(!boss||boss.hp<=0)return;
 goldNpcTick(s,actors);
 const elapsed=s.clock-s.combat.startedAt;
 const adds=s.combat.enemies.filter(e=>e.id!==boss.id&&e.hp>0);
 const tanks=living.filter(c=>combatRole(c)==='tank');
 for(const c of living){
  c.raidTargetId=combatRole(c)==='tank'?(c.raidMainTank||!adds.length?boss.id:adds[0].id):(raid.tactics.focusAdds&&adds.length?adds[0].id:boss.id);
 }
 // Hard enrage bounds attempts and makes insufficient throughput observable.
 if(elapsed>=180000){raidNotice(s,'首领狂暴：战斗超过180秒，全团覆灭。','raid-failure');for(const c of living)hurt(s,boss,c,c.hp*10,'狂暴',{school:2});return;}
 if(raid.id==='lucifron'){
  ...
  for(const fire of raid.fires){
   ...
   if(raid.tactics.avoidFire)for(const c of living.filter(c=>distance(c,fire)<=fire.radius+1&&!controlled(c,s.clock)&&goldAvoidsFire(s,c,fire))){
    ...
    c.cast=null;moveToward(s,c,{position:fire.position+Math.cos(angle)*10,positionY:fire.positionY+Math.sin(angle)*10},0,s.clock);c.raidEvadingAt=s.clock;
   }
  }
 ...
 for(const c of living){
  ...
  if(raid.tactics.dispel&&[5,8].includes(c.classId)){
   ...
   if(sp&&supportCast(s,c,sp,target,()=>{raid.support.dispels+=dispelSpellAuras(target,[type],1,s,'negative');},type===1?'驱散末日':'解除诅咒'))continue;
  }
