...
import {executeSpellEffects} from './spell-effect-runtime.js';
...
function amount(c,sp,n=1){const raw=effectRange(c,sp,n)[0]+(sp['EffectPointsPerComboPoint'+n]||0)*(c.combo||0);return [2,9,10,17,31,58,121].includes(sp['Effect'+n])||[3,8,53,161].includes(sp['EffectApplyAuraName'+n])?raw:talentSpellValue(c,sp,8,raw);}
...
export function prepareClassAbility(s,c,e,sp,actors){
 ...
 if(name.endsWith('Trap')&&(c.trap?.until>s.clock||s.clock>s.combat.startedAt+100&&s.combat.pull?.engagedAt!=null&&!(c.feignUntil>s.clock)))return null;
...
export function genericEffects(s,c,t,sp,actors,api,options={}){
 ...
 return executeSpellEffects(s,c,t,sp,actors,api,{amount,duration,damage,heal,aura,dispelClassEffects},options);
}
export function executeExtendedClassEffect(s,c,target,sp,actors=[c],api={}){
 ...
 if(name==='Charge'||name==='Intercept'||name==='Feral Charge'){const p=point(target);setCombatPosition(s,c,{x:p.x-3,y:p.y});genericEffects(s,c,target,sp,actors,api);return true;}
 if(name==='Blink'){setCombatPosition(s,c,{x:point(c).x+20,y:point(c).y});c.rootUntil=0;c.stunUntil=0;c.auras=(c.auras||[]).filter(a=>![12,26].includes(a.type));return true;}
 ...
 if(['Preparation','Readiness'].includes(name)){resetSpellCooldowns(c,spell=>spell&&spell.Id!==sp.Id&&(name==='Readiness'?spell.SpellFamilyName===9:spell.SpellFamilyName===8));return true;}
