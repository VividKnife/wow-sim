...
export const raidLoot={lucifron:[991001,991002,991003,991004,991005,991006],magmadar:[991011,991012,991013,991014,991015,991016]};
...
export function enterGuildRaid(s){
 ...
 const r=s.guildRaid??={version:1,week,cleared:[],claims:{},tactics:{...defaultRaidTactics},attempts:[],rewards:[]};
 if(r.week!==week){r.week=week;r.cleared=[];r.claims={};r.rewards=[];r.attempts=[];}
 r.active=true;r.activeBoss=null;r.recoverUntil=0;r.coreIds=[s,...s.party].map(c=>c.id);
 ...
 log(s,'熔火之心公会远征：核心5人、公会20人已集结。','raid');
}
export function leaveGuildRaid(s){
 const r=camp(s);r.active=false;r.recoverUntil=0;
 s.party=s.party.filter(c=>!c.guildUnit);s.activity={type:'idle'};
 log(s,'已返回营地外，熔火之心进度与战利品已保存。','raid');
}
...
export function guildRaidAction(s,a){
 ...
 if(a.type==='raidTactics'){
  ...
  r.recoverUntil=s.clock+10000;s.activity={type:'raidRecovery',startedAt:s.clock,endsAt:r.recoverUntil,reason:'公会医护正在复活并补充食物、饮水与弹药。'};

 }else if(a.type==='raidRestart'){
  if(r.cleared.length<2)throw new Error('完成本次远征后才能重新挑战。');
  r.cleared=[];r.activeBoss=null;log(s,'开始本周练习远征，已领取过的首领奖励不会重复发放。','raid');
 }else if(a.type==='raidStart'){
  const boss=moltenCoreBosses.find(b=>b.id===a.bossId),index=moltenCoreBosses.indexOf(boss);
  if(!boss||r.cleared.includes(boss.id)||index>0&&!r.cleared.includes(moltenCoreBosses[index-1].id))throw new Error('首领未解锁或已击败。');
  ...
  if(actors.some(c=>c.hp<=0))throw new Error('有成员倒下，请先在营地休整。');
  if(s.pending.length)throw new Error('请先领取待拾取战利品。');
  // Every pull restores resources; equipment and strategies remain owned.
  for(const c of actors)restoreRaidMember(c,s);
  r.activeBoss=boss.id;s.activity={type:'idle',reason:'团队正在挑战'+boss.name};
  beginMoltenCoreBattle(s,boss.id,r.tactics);
  s.combat.requiresManualControl=true;
...
export function settleGuildRaid(s){
 ...
 if(s.combat||!b?.raidEncounter||b.guildSettled)return;
 b.guildSettled=true;
 ...
 if(won){
  if(!r.cleared.includes(bossId))r.cleared.push(bossId);
  if(!r.claims[bossId]){
  ...
  }else log(s,'练习战完成：本周该首领奖励已领取。','raid');
  s.activity={type:s.hp>0?'idle':'dead',reason:'首领已击败，领取战利品并休整后继续。'};
...
export function guildRaidView(s){
 ...
 return {id:MOLTEN_CORE_ID,name:'熔火之心',active:live,minimumLevel:60,capacity:25,canEnter:s.level===60&&s.growthPolicy!=='companion'&&!live&&!s.combat&&!s.dungeon&&s.party.length===4&&[s,...s.party].every(c=>c.level===60&&c.hp>0),
  weekResetAt:(week+1)*WEEK+345600000,cleared:current?r?.cleared||[]:[],claims:current?r?.claims||{}:{},tactics:r?.tactics||defaultRaidTactics,
  recovering:!!r?.recoverUntil,remaining:Math.max(0,(r?.recoverUntil||0)-s.clock),activeBoss:r?.activeBoss,bosses:moltenCoreBosses.map(b=>({...b,loot:raidLoot[b.id].filter(id=>items[id]).map(id=>({id,name:nameOf('items',id)}))})),squadNames:guildSquadNames,
  ...
  members:actors.map((c,i)=>({id:c.id,name:c.name,classId:c.classId,role:combatRole(c),squad:Math.floor(i/5),guild:!!c.guildUnit,hp:c.hp,maxHp:stats(c).maxHp,mana:c.mana,maxMana:stats(c).maxMana})),events:enc?.events?.slice(-8)||[],fires:enc?.fires||[],nextMechanics:s.combat&&enc?(enc.id==='lucifron'?[{name:'末日',at:enc.nextDoom},{name:'诅咒',at:enc.nextCurse}]:[{name:'狂暴',at:enc.nextFrenzy},{name:'恐慌',at:enc.nextFear},{name:'熔岩',at:enc.nextBomb}]):[]};
}
