...
export function extendedMoltenCoreTick(s,actors,boss,hurt,targets){
 ...
 const fire=(units,radius=6)=>{for(const c of units)r.fires.push({id:`fire-${s.clock}-${c.id}`,position:c.position,positionY:c.positionY,radius,armedAt:s.clock+2500,until:s.clock+11000,next:s.clock+2500});};
 if(r.kind==='trash'){
 ...
 if(r.id==='garr')for(const e of s.combat.enemies.filter(e=>e.id!==boss.id&&e.hp<=0&&!r.deadAdds.includes(e.id))){r.deadAdds.push(e.id);boss.low*=1.12;boss.high*=1.12;for(const c of living.filter(c=>distance(c,e)<12))hit(c,650,'火誓者爆炸');raidNotice(s,'火誓者爆炸，加尔的攻击增强。');}
 ...
 if(r.id==='ragnaros'){
  if(!r.submerged&&s.clock>=r.nextSubmerge){
   ...
   for(let i=0;i<6;i++){const e=raidEnemy(s,`son-${s.clock}-${i}`,'烈焰之子',12143,10500,300,430);e.position=22;e.positionY=(i-3)*4;e.target=living.find(c=>combatRole(c)==='tank'&&!c.raidMainTank)?.id||s.id;e.threat[e.target]=2500;s.combat.enemies.push(e);}
 ...
 if(s.clock>=r.nextSpecial){r.nextSpecial+=15000;
  ...
  if(r.id==='gehennas'){curse(8,19716,{type:118,amount:-60});fire(targets(s,living,2));raidNotice(s,'基赫纳斯施放减疗诅咒与火雨。');}
  ...
  if(r.id==='majordomo'){fire(targets(s,living,2));raidNotice(s,'烈焰议会施放熔岩冲击。');}
  if(r.id==='ragnaros'){fire(targets(s,living,3));const tank=living.find(c=>c.id===boss.target);if(tank){hit(tank,800,'拉格纳罗斯之怒');tank.position=Math.max(-15,tank.position-14);boss.threat[tank.id]=Math.max(0,(boss.threat[tank.id]||0)*.5);}raidNotice(s,'拉格纳罗斯之怒：击退当前坦克，熔岩在后排爆发。');}
 }
 if(s.clock>=r.nextPulse){r.nextPulse+=12000;
  if(r.id==='baron-geddon'){raidAnimation(s,boss);fire([boss],12);raidNotice(s,'地狱火：撤离男爵周围。');}
  if(r.id==='shazzrah')for(const c of living.filter(c=>distance(c,boss)<16))hit(c,500,'魔爆术',6);
  if(r.id==='golemagg'&&boss.hp<boss.maxHp*.2){for(const c of living)hit(c,260,'地震',0);raidNotice(s,'古雷曼格濒死狂怒，地震震动全团。');}
 }
