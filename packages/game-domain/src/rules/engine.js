...
import {storageActions,storageAction,sellBatch,settleAuctions,marketView,marketPrice,bankHere,bankCapacity,protectedItem,tradable,bankable} from './inventory.js';
...
function tick(s){s.time=s.clock;const st=stats(s),wasAlive=s.hp>0;environmentTick(s,{stats,hurtPlayer});
 ...
 if(s.combat){combatTick(s);if(!s.combat&&s.hp>0&&s.settings.autoLoot)collectLoot(s);recordDungeonProgress(s);advanceDungeon(s);escortTick(s);return;}
