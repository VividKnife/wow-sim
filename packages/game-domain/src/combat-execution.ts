...
import {account, owned} from './context.ts';
...
export async function prepareCombatPlan(this: GameService, table: 'activities' | 'instances', id: string, now: number) {
    const input = await this.store.transaction(async tx => {
        ...
        const presence = await account(tx, accountId);
        if (now - presence.lastSeenAt >= 5_000) return null;
        ...
        return {owner, state, deadline, accountId};
    });
    if (!input) return;
    ...
    await this.store.transaction(async tx => {
        const owner = await tx.get<Activity | Instance>(table, id);
        if (!owner || owner.status !== 'running' || owner.playback || owner.simulationVersion !== input.owner.simulationVersion) return;
