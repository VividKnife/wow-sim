...
try {
  ...
  assert.equal((await request('/api/game', {headers: {'oai-authenticated-user-id': 'forged', 'oai-authenticated-user-email': 'fake@example.com'}})).status, 401);
  ...
  const creation = await request('/api/game', {method: 'POST', cookie, body: {type: 'create', name: '部署验证', classId: 1, raceId: 1, requestId: randomUUID()}});
  ...
  const restore = await request('/api/game', {cookie});
  assert.equal(restore.status, 200);
  const saved = await restore.json();
  assert.equal(saved.snapshot.player.id, first.snapshot.player.id);
  assert.equal((await request('/api/game', {method: 'POST', cookie, requestOrigin: 'https://evil.example', body: {type: 'advance'}})).status, 403);
  const other = await request('/api/auth/register', {method: 'POST', body: {...credentials, username: 'second_account'}});
  assert.equal(other.status, 200);
  const otherCookie = other.headers.get('set-cookie').split(';')[0];
  const isolated = await request('/api/game', {cookie: otherCookie});
  assert.equal((await isolated.json()).snapshot, null);
  assert.equal((await request(`/api/game?characterId=${encodeURIComponent(first.snapshot.player.id)}`, {cookie: otherCookie})).status, 404);
  assert.equal((await request('/api/auth/logout', {method: 'POST', cookie})).status, 200);
  assert.equal((await request('/api/game', {cookie})).status, 401);
  const login = await request('/api/auth/login', {method: 'POST', body: credentials});
  assert.equal(login.status, 200);
  const restored = await request('/api/game', {cookie: login.headers.get('set-cookie').split(';')[0]});
