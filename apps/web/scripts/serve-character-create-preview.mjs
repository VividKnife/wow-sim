...
const server=await createServer({configFile:false,root:app+'test/browser',publicDir:app+'public',plugins:[react(),{name:'read-only-model-preview',configureServer(server){server.middlewares.use(async(req,res,next)=>{
 ...
 if(url.pathname==='/api/game'){const tauren=url.searchParams.get('saveId')==='tauren-fixture',preview=characterPreview(tauren?6:1,tauren?1:8,tauren?1:20);res.setHeader('Content-Type','application/json');res.end(JSON.stringify({contentVersion:tauren?'tauren-fixture':'human-fixture',snapshot:{player:preview,view:{location:{name:tauren?'北郡':'闪金镇'}}}}));return;}
 if(url.pathname==='/api/game/content'){const tauren=url.searchParams.get('version')==='tauren-fixture',preview=characterPreview(tauren?6:1,tauren?1:8,tauren?1:20);res.setHeader('Content-Type','application/json');res.end(JSON.stringify({contentVersion:tauren?'tauren-fixture':'human-fixture',items:preview.items}));return;}
 ...
 if(url.pathname==='/api/character-preview'){try{res.setHeader('Content-Type','application/json');res.end(JSON.stringify(characterPreview(Number(url.searchParams.get('raceId')),Number(url.searchParams.get('classId')),Number(url.searchParams.get('level')))));}catch{res.statusCode=400;res.end('{}');}return;}next();
