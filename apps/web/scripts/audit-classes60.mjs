...
import {classDefinitions,classAbilities,classContentManifest,classTalentTrees,talents} from '../lib/game/catalog.js';
import {supportedSpellNames,talentExecutionCoverage} from '../lib/game/class-support.js';
import {extendedSpellNames} from '../lib/game/class-spell-registry.js';
import {utilitySpellNames,passiveSpellNames} from '../lib/game/class-utility-data.js';
import {talentActiveNames} from '../lib/game/talent-runtime.js';
import {racialActiveNames,racialPassiveNames} from '../lib/game/racial-effects.js';
import {petTrainerAbilities} from '../lib/game/pet-knowledge.js';
...
const registries=[['extended-class-spell',extendedSpellNames,'apps/web/lib/game/class-spell-effects.js'],['class-utility',utilitySpellNames,'apps/web/lib/game/class-utility.js'],['passive-or-pet-command',passiveSpellNames,'apps/web/lib/game/class-utility-data.js'],['talent-active',talentActiveNames,'apps/web/lib/game/talent-runtime.js'],['racial-active',racialActiveNames,'apps/web/lib/game/racial-effects.js'],['racial-passive',racialPassiveNames,'apps/web/lib/game/racial-effects.js']];
function execution(name){const registered=registries.filter(([,names])=>names.has(name)).map(([registry,,module])=>({registry,module}));if(!registered.length&&supportedSpellNames.has(name))registered.push({registry:'legacy-class-handler',module:'apps/web/lib/game/class-mechanics.js'});return{registrySupported:supportedSpellNames.has(name),routes:registered,verification:'registry-reconciled; behavior verified by family regression tests, not individual spell parity'};}
...
export function createCoverage(){
 ...
 return{
  ...
  source:{bundle:'apps/web/data/classes-reference.json',sha256:createHash('sha256').update(readFileSync(new URL('../data/classes-reference.json',import.meta.url))).digest('hex'),petFamilyBundle:'apps/web/data/pet-family-reference.json',petFamilySha256:createHash('sha256').update(readFileSync(new URL('../data/pet-family-reference.json',import.meta.url))).digest('hex'),manifestSchemaVersion:classContentManifest.schemaVersion},
