...
const server=await createServer({configFile:false,root:app+'test/browser',publicDir:app+'public',plugins:[react(),modelAssets],resolve:{alias:{'@':app}},css:{postcss:{plugins:[tailwind()]}},server:{hmr:false,host:'127.0.0.1',port,strictPort:true,fs:{allow:[app]}}});
