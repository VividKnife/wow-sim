...
const server=await createServer({configFile:false,root:app+'test/browser',publicDir:app+'public',plugins:[react(),modelAssets],json:{stringify:true,namedExports:false},resolve:{alias:{'@':app}},css:{postcss:{plugins:[tailwind()]}},server:{hmr:false,watch:{ignored:['**/classic-battle-models-manifest.json','**/classic-characters-manifest.json','**/public/creatures/**','**/public/characters/**']},host:'127.0.0.1',port,strictPort:true,fs:{allow:[app]}}});
