...
const api={name:'local-simulation-preview',configureServer(server){server.middlewares.use((req,res,next)=>{
 ...
 void(async()=>{
  ...
  if(url.pathname==='/api/game/content')data=contentPack(clientContent(),url.searchParams);
   ...
   const input=JSON.parse(body);data=await activeService.localSimulation(accountId,input);
   ...
   let snapshot;
   if(req.method==='POST'){let body='';for await(const chunk of req)body+=chunk;snapshot=await activeService.command(accountId,JSON.parse(body));commands++;}
   else snapshot=await activeService.snapshot(accountId,url.searchParams.get('characterId')||undefined,true);
