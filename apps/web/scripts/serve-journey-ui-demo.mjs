...
const server=await createServer({configFile:false,root:app+'test/browser',publicDir:app+'public',plugins:[react(),{name:'journey-ui-assets',configureServer(server){server.middlewares.use(async(req,res,next)=>{
 ...
 });}}],resolve:{alias:{'@':app}},optimizeDeps:{entries:['journey-ui.html']},css:{postcss:{plugins:[tailwind()]}},server:{hmr:false,host:'127.0.0.1',port:5199,strictPort:true,fs:{allow:[fileURLToPath(new URL('../../../',import.meta.url))]}}});
await server.listen();console.log('Journey UI V2: http://127.0.0.1:5199/journey-ui.html');
