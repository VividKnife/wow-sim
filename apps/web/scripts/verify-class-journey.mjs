...
import {createGame,act,advance,stats,shop,view} from '../../../packages/game-domain/src/rules/engine.js';
import {quests,items,creatures,creatureLoot,monsterIdsAt,nodes,route,abilities,trainerNodes,spells,talents,classAbilities} from '../../../packages/game-domain/src/rules/catalog.js';
...
const targetLevel=Number(process.argv[2]||20),seed=Number(process.argv[3]||283),classId=Number(process.argv[5]||8),raceId=({3:3,7:2,11:4})[classId]||1;
...
let s=createGame('职业验收'+classId,seed,0,{classId,raceId}),commands=0,lastLevel=1,iterations=0;
const summary={seed,targetLevel,classId,raceId,method:'Only normal game commands; simulated elapsed time; no granted levels, currency, gear, skills or healing.',checkpoints:[],failures:[],deferred:[]};
const write=row=>appendFileSync(journal,JSON.stringify(row)+'\n');
write({type:'create',name:s.name,seed,now:0,options:{classId,raceId}});
function checkpoint(){
...
function score(id){const i=items[id];if(!i||!canEquip(s,i)||!i.InventoryType)return -1;let score=i.armor*.04+(i.dmg_min1+i.dmg_max1)/2/(i.delay/1000||1);for(let n=1;n<=10;n++)score+=( ([1,2].includes(classId)?{4:3,7:2,3:1}: [3,4].includes(classId)?{3:3,7:2,4:1}:{5:3,7:2,6:1.5})[i['stat_type'+n]]||.05)*i['stat_value'+n];return score;}
function equip(){for(const i of [...s.bag]){const data=items[i.id];if(data.class===1&&data.ContainerSlots&&!data.BagFamily&&(s.bags.length<4||data.ContainerSlots>Math.min(...s.bags.map(b=>items[b.id].ContainerSlots)))){command({type:'equipBag',uid:i.uid});continue;}if(score(i.id)>score(s.equipment[slotOf(data)]?.id)&&score(i.id)>=0){if(slotOf(data)===17&&items[s.equipment[16]?.id]?.InventoryType===17)continue;command({type:'equip',uid:i.uid});}}}
function questViews(){return Object.keys(quests).map(id=>questProgress(s,+id));}
function sell(){
 if(!shop(s).length)return;
 const needed=new Map();for(const q of questViews().filter(q=>!q.completed))for(const o of q.objectives.filter(o=>o.kind==='item'))needed.set(o.id,Math.max(needed.get(o.id)||0,o.required));
 ...
 for(const i of [...s.bag]){const item=items[i.id],aura=spells[item.spellid_1]?.EffectApplyAuraName1;
  const keepForQuest=(needed.get(i.id)||0)>0;needed.set(i.id,Math.max(0,(needed.get(i.id)||0)-i.count));
  if(item.SellPrice>0&&!protectedItem(i)&&!keepForQuest&&!item.startquest&&(![84,85].includes(aura)||!provisionIds.has(i.id))&&item.class!==12)command({type:'sell',uid:i.uid});
 }
...
function train(){if(!trainerNodes.includes(s.location))return;
 const stamp=s.level+':'+s.money+':'+s.learned.length;if(lastTraining===stamp)return;
 for(const a of (classAbilities[classId]||[]).filter(a=>a.requiredLevel<=s.level&&!s.learned.includes(a.spellId)).sort((a,b)=>a.requiredLevel-b.requiredLevel||a.costCopper-b.costCopper)){
  if(a.acquisition==='talent'||a.costCopper>s.money)continue;
  try{command({type:'train',id:a.spellId});}catch(error){if(!/前一等级|技能书|种族|天赋|尚未实现|已经学会|前置|解锁/.test(error.message))throw error;}
 }
 lastTraining=s.level+':'+s.money+':'+s.learned.length;
}
function talentsAndBuffs(){
 const desired=({1:['Cruelty','Deflection','Improved Rend'],2:['Divine Strength','Improved Seal of Righteousness'],3:['Improved Aspect of the Hawk','Lethal Shots'],4:['Improved Sinister Strike','Malice','Improved Eviscerate'],5:['Spirit Tap','Improved Shadow Word: Pain','Shadow Focus'],7:['Convection','Concussion'],8:['Improved Fireball','Ignite','Improved Fire Blast'],9:['Improved Corruption','Demonic Embrace','Improved Imp'],11:['Improved Wrath','Improved Moonfire']})[classId];
 for(const name of desired){const t=Object.values(talents).find(t=>t.classId===classId&&t.name===name);if(!t)continue;while(Object.values(s.talents).reduce((a,b)=>a+b,0)<s.level-9&&(s.talents[t.id]||0)<t.maxRank){try{command({type:'talent',id:t.id});}catch{break;}}}
}
...
try{
 ...
 while(s.level<targetLevel&&iterations++<20000){
  ...
  const affordable=(classAbilities[classId]||[]).some(a=>a.requiredLevel<=s.level&&!s.learned.includes(a.spellId)&&s.money>=a.costCopper&&!['talent','book','racial'].includes(a.acquisition));
 ...
 travel('northshire');train();talentsAndBuffs();recover();
 summary.skills=view(s).skills.filter(a=>a.requiredLevel<=20).map(a=>({id:a.spellId,name:a.name,known:a.known,reason:a.blockedReason}));
