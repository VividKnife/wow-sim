...
const server=await createServer({configFile:false,root:app+'test/browser',publicDir:app+'public',plugins:[react(),{name:'world-scene-assets',configureServer(server){server.middlewares.use(async(req,res,next)=>{
 ...
 });}}],resolve:{alias:{'@':app}},optimizeDeps:{entries:['world-scene.html']},css:{postcss:{plugins:[tailwind()]}},server:{host:'127.0.0.1',port:5194,strictPort:true,fs:{allow:[fileURLToPath(new URL('../../../',import.meta.url))]}}});
await server.listen();console.log('World scene demo: http://127.0.0.1:5194/world-scene.html');
