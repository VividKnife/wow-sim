...
async function render(items,revision){
 ...
 try{
  ...
  viewer=new window.ZamModelViewer({type:2,container:window.jQuery(host),aspect:host.clientWidth/host.clientHeight,contentPath:ROOT,models:{id:1,type:16},items:items.filter(i=>displays.has(i.id)).map(i=>[i.slot,displays.get(i.id)]),dataEnv:'classic',env:'classic',gameDataEnv:'classic',hd:false,cls:8,transparent:true});
...
window.addEventListener('message',event=>{
 ...
 void render(message.items,message.revision);
});
