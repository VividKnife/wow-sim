...
export function startRecovery(s){let needed=false;const missing=[];
 for(const c of recoveryMembers(s)){
  ...
  if(foodNeeded&&!food||waterNeeded&&!water){missing.push(c.name);continue;}
 ...
 if(missing.length){s.activity={type:'idle',reason:'补给不足：'+missing.join('、')+'。购买或制造食物和水后继续。'};log(s,'自动活动停止：补给不足','stop');}
 return needed;
}
