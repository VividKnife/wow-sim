...
import passives from '../../data/companion-passive-reference.json' with {type:'json'};
import {activeAuras,armorWithAuras,hasAura} from './combat-auras.js';
import {distance,moveToward,inSpellRange,detectsTarget} from './combat-space.js';
...
export function interruptPriestForRescue(s,c,actors){
 if(c.classId!==5||!c.cast||c.cast.until<=s.clock)return;
...
function decidePriest(s,c,enemies,actors,api){
 ...
 const target=injuredAllies(actors)[0];
 if(target){
 ...
 // Preserve mana for the next injury; only help with damage above 70% mana.
 ...
 for(const rule of companionRules(c)){
  ...
  if(!sp||sp.SpellName!=='Smite'||c.mana<Math.max(sp.mana,stats(c).maxMana*.7)||(c.cooldowns[id]||0)>s.clock||!strategyAllows(s,c,e,sp,rule))continue;
