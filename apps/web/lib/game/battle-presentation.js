import {classCombatMeta,classResource} from '../class-combat.js';
...
export function battlePresentation(s){
 ...
 for(const actor of all){
  ...
  const cooldowns=Object.entries(actor.cooldowns||{}).filter(([id,until])=>until>clock&&spells[id]).map(([id,readyAt])=>({...spellDetail(id),readyAt})).sort((a,b)=>a.readyAt-b.readyAt);
