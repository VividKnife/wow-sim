...
export function dungeonView(s){
 const active=!!s.dungeon,run=s.dungeon||s.dungeonSave,encounter=run&&dungeonRoute[run.cursor];
 ...
 return {active,saved:!!s.dungeonSave,canReset:!dungeonResetReason(s),resetReason:dungeonResetReason(s),atEntrance:s.location==='deadmines',name:'死亡矿井',minimumLevel:10,recommendedLevel:18,
  ...
  route:dungeonRoute.map((e,i)=>({id:e.id,name:e.nameZh,kind:e.kind,optional:!!e.optional,status:run?.cleared[e.id]?'cleared':run?.skipped[e.id]?'skipped':run&&i===run.cursor?'current':'ahead'}))};
}
