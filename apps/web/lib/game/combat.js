...
import {point,distance,detectsTarget,moveToward,moveAway,areaTargets,castRange,inSpellRange,groundArea,selfArea,spellRadius,aliveEnemy} from './combat-space.js';
...
import {activeAuras,controlled,rooted,hasAura,movementMultiplier,addMovementSlow,addCombatAura,attackTimeMultiplier,schoolImmune,physicalDamageBonus} from './combat-auras.js';
...
function decideMage(s,c,focus){
 for(const rule of c.rules||defaultRules){if(!rule.enabled)continue;const id=knownRank(c,rule.spell);if(!id)continue;const sp=spellInfo(c,id),e=sp.SpellName==='Polymorph'?polymorphTarget(s,c,focus,sp,rule):focus;if(!e||!ruleMatches(s,c,e,rule,sp)||!strategyAllows(s,c,e,sp,rule))continue;
...
export function combatTick(s){
 ...
 for(const c of actors.filter(c=>c.hp>0)){
  ...
  else if(!c.cast&&!c.stealthed&&c.classId!==5&&!(c.classId===8&&s.clock<c.nextAction)&&(![7,9,11].includes(c.classId)||c.form||s.clock>=c.nextAction))melee(s,c,e);
 }
