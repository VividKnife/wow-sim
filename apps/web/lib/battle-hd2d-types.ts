...
export type BattleUnitData={id:string;name:string;hp:number;maxHp:number;entry?:number;stunUntil?:number;rootUntil?:number;modelAnimation?:{action:string;startedAt:number;until:number};classId?:number;raceId?:number;gender?:'male'|'female';level?:number;creatureType?:number;rank?:number;form?:string;kind?:string;foe?:boolean;removed?:boolean;marker?:'focus'|'control';commandMark?:string;petUnit?:boolean;totemUnit?:boolean;cast?:BattleCast;swing?:number;target?:string;polyUntil?:number;visual?:{model?:CreatureModelData;kind?:string;src?:string;species?:string;creatureType?:number}};
