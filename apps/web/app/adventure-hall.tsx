...
export default function AdventureHall({state:s,data:d,busy,send,compact=false}:GameProps&{compact?:boolean}){
 ...
 return <section className="panel adventure-hall" aria-label="副本组队大厅">
  <header className="hall-heading"><div><span className="eyebrow">冒险者大厅</span><h2>副本出征名单</h2><p>自有队友与 NPC 玩家自由混编。出征名单独立保存，野外队伍不变。</p></div><span className="hall-count"><UsersRound size={20}/>{selected.length+1}<small>/ 5</small></span></header>
  ...
  {!w.ready?<div className="hall-welcome"><BookOpen size={26}/><div><h3>招募副本队员</h3><p>每批可招募 6 名 NPC，涵盖坦克、治疗和输出。添加好友后可再次邀请。</p></div><Button disabled={locked} onClick={()=>send({type:'npcVisit'})}>走进冒险者大厅</Button></div>:<>
   ...
   {expanded&&<>
    ...
    {!rows.length&&<div className="hall-notice"><p>{pools[scope].length?'当前名单没有符合筛选条件的冒险者。':scope==='friends'?'还没有好友。在本批冒险者的卡片上点击星标，留下下次同行的约定。':scope==='recent'?'还没有共同冒险记录。邀请 NPC 进入副本后，他们会出现在这里。':'尚未选择 NPC。可以与自有队友混编，也可以组成全 NPC 小队。'}</p>{pools[scope].length?<Button variant="ghost" size="sm" onClick={()=>{setQuery('');setFilter('all');setProfession('all');}}>清除筛选</Button>:<Button variant="ghost" size="sm" onClick={()=>setScope('board')}>认识本批冒险者</Button>}</div>}
