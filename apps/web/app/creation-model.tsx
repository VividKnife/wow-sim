...
export default function CreationModel({raceId,classId,gender,boost,raidReady=false,fallback,title}:{raceId:number;classId:number;gender:'male'|'female';boost:boolean;raidReady?:boolean;fallback:ReactNode;title:string}){
 const [result,setResult]=useState<Preview|null>(null),[error,setError]=useState(false),[attempt,setAttempt]=useState(0);
 useEffect(()=>{
  const controller=new AbortController();
  fetch(`/api/character-preview?raceId=${raceId}&classId=${classId}&level=${raidReady?60:boost?20:1}`,{signal:controller.signal})
   .then(async response=>{if(!response.ok)throw new Error('Preview unavailable');return response.json();})
   .then(data=>{if(!controller.signal.aborted)setResult(data);})
   .catch(()=>{if(!controller.signal.aborted)setError(true);});
  return()=>controller.abort();
 },[raceId,classId,boost,raidReady,attempt]);
