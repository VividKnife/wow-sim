...
export default function DungeonPage(props:GameProps&{onOpenParty:()=>void;onConfigure:()=>void;onObserve:()=>void}){
 ...
 return <div className="dungeon-page">
  ...
  {!expedition&&<AdventureHall {...props} compact/>}
  ...
  {expedition&&<QuestScenes {...props}/>}<section className="panel dungeon-journal" aria-label="地下城手册">
   {selected?<>
    ...
    <header className={'journal-detail-header '+(selected.playable?'is-playable':'')} style={{backgroundImage:`linear-gradient(90deg,rgba(17,15,12,.96),rgba(17,15,12,.48)),url(${selected.background})`}}><div className="journal-emblem" aria-hidden="true">⚔</div><div><div className="eyebrow">{selected.zone} · {selected.groupSize||5} 人地下城</div><h2>{selected.name}</h2><p>{selected.description}</p><div className="journal-tags"><span>最低等级 {selected.minimumLevel}</span><span>建议等级 {selected.recommendedLevel}</span><span className={selected.playable?'journal-playable':'journal-info'}>{selected.playable?'可挑战':'仅供查阅 · 尚未开放挑战'}</span></div></div></header>
  ...
  {!expedition&&<section className="panel" aria-label="地下城准备"><div className="section-heading"><h2>出发检查</h2><small>建议搭配坦克、治疗与输出</small></div><div className="dungeon-preparation">
   <div><small>小队成员</small><strong>{(d.npcWorld?.selected.length??s.party.length)+1} / 5 人</strong><Button variant="outline" onClick={onOpenParty}>管理队伍</Button></div>
