...
function Spellbook({state:s,data:d,busy,send}:GameProps){
 ...
 return <><ClassCompanion state={s} data={d} busy={busy} send={send}/><section className="spellbook-shell" aria-label={`${d.className}法术书`}>
  <nav className="spellbook-tabs" aria-label="法术书技能系">{tabs.map(tab=>{const sample=tab.skills[0];return <button type="button" key={tab.id} className={activeTab===tab.id?'active':''} aria-pressed={activeTab===tab.id} onClick={()=>setBookSection(tab.id)} title={`${tab.label} · ${tab.skills.length}`}><span className="spellbook-tab-icon">{sample?.icon?<Icon src={sample.icon} name={tab.label} size={34}/>:d.className?.slice(0,1)}</span><span>{tab.label}<small>{tab.skills.length}</small></span></button>;})}</nav>
  <div className="spellbook-frame">
   ...
   <div className="spellbook-toolbar"><input aria-label="搜索职业技能" placeholder="搜索法术与技能" value={skillSearch} onChange={e=>setSkillSearch(e.target.value)}/><div className="spellbook-filters" aria-label="学习状态筛选">{['全部','可学习','已学习','未学习'].map(value=><button type="button" className={value===skillFilter?'active':''} aria-pressed={value===skillFilter} key={value} onClick={()=>setSkillFilter(value)}>{value}</button>)}</div><label>目标<select aria-label="技能目标" value={targetId} onChange={e=>setTargetId(e.target.value)}>{[s,...s.party,...(s.pet?[s.pet]:[])].map((c:any)=><option key={c.id} value={c.id}>{c.name}{c.hp<=0?'（已死亡）':''}</option>)}{(s.combat?.enemies||[]).filter((e:any)=>e.hp>0).map((e:any)=><option key={e.id} value={e.id}>{e.name}</option>)}{!s.combat&&d.monsters.map((e:any)=><option key={e.id} value={'npc:'+e.id}>{e.name}</option>)}{s.groundEffects?.filter((e:any)=>e.trap&&e.until>s.clock).map((e:any)=><option key={e.id} value={e.id}>陷阱 · {e.name||e.id}</option>)}{d.lockTargets?.map((i:any)=><option key={i.id} value={i.id}>{i.name}</option>)}</select></label></div>
   <div className="spellbook-guidance"><p>{d.canTrain?'在世界页面点击训练师，查看并学习新技能。':'前往北郡、闪金镇或暴风城的训练地点学习新技能。'}</p><p>伤害、治疗和吸收显示基础效果；最终数值受装备、天赋和目标减伤影响。</p></div>
