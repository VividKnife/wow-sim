...
export default function City(props:GameProps){
 ...
 return <section className="city" aria-label={city.name+'主城'}>
  ...
  <nav className="city-district-nav" aria-label={city.name+'城区列表'}>{districts.map((district,index)=><button key={district.id} aria-pressed={selected.id===district.id} onClick={()=>choose(district.id)} className={selected.id===district.id?'active':''}><small>{String(index+1).padStart(2,'0')}</small>{district.name}</button>)}</nav>
