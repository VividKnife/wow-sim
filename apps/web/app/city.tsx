...
const glyphs:Record<string,string>={bank:'▣',auction:'⚖',shop:'◇',inn:'⌂',trainer:'✧',professions:'⚒',flight:'↗',tram:'⇄',quests:'!'};
...
export default function City(props:GameProps){
 ...
 return <section className="city" aria-label="暴风城主城">
  ...
  <div className="city-guide"><span>✧ 卫兵指路</span><label><span className="sr-only">寻找主城服务</span><select aria-label="寻找主城服务" value={filter} onChange={e=>e.target.value==='all'?setFilter('all'):guide(e.target.value)}><option value="all">你想去哪里？</option>{[['bank','银行'],['auction','拍卖行'],['trainer',d.className+'训练师'],['professions','生活职业与工坊'],['inn','旅店'],['shop','商人'],['flight','狮鹫管理员'],['tram','矿道地铁']].map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><p>{filter==='all'?'点选城区查看服务，或请卫兵为你指路。':`已标出目的地：${selected.name}。${here?'你就在附近。':'抵达后即可办理。'}`}</p><button className="city-text-button" onClick={()=>{setFilter('all');choose(s.location);}}>定位自己</button></div>
  ...
  {active&&<div className="city-service-panel" ref={serviceRef} aria-label={active.name+'服务面板'}><div className="city-npc"><span className="city-npc-emblem">{glyphs[active.id]}</span><div className="grow"><div className="city-kicker">{current.name} / {active.name}</div><h2>{active.npc}</h2><p>“{active.greeting}”</p></div><Button variant="ghost" aria-label="关闭主城服务" onClick={()=>setOpened(null)}>关闭 ×</Button></div><CityServicePanel key={active.id+':'+s.location} service={active} {...props}/></div>}
