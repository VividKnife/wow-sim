...
function ItemSlot({instance,item,label,glyph:Glyph=Backpack,selected,onSelect,use,busy,onUse}:{instance?:Instance;item?:ItemData;label:string;glyph?:typeof Shirt;selected?:boolean;onSelect?:(uid:string)=>void;use?:{canUse:boolean;reason:string;label:string;description:string};busy?:boolean;onUse?:(uid:string)=>void}){
 ...
 return <button type="button" className={'armory-slot rarity-border-'+item.quality+(selected?' selected':'')+(use?' usable-slot':'')} aria-disabled={use&&instance.id!==6948?busy||!use.canUse:undefined} aria-label={`${label}：${item.name}${instance.count>1?' ×'+instance.count:''}`} aria-pressed={!!selected} onClick={()=>{onSelect?.(instance.uid);if(instance.id!==6948&&use?.canUse&&!busy)onUse?.(instance.uid);}}>
  ...
  <span className="armory-tooltip"><ItemFacts item={item} instance={instance}/><span className="tooltip-hint">{instance.id===6948?'点击选中，再点击使用炉石':use?(use.reason||`点击${use.label} · ${use.description}`):'点击查看详情'}</span></span>
 </button>;
}
 ...
 return <div className="armory-layout">
  ...
  <div className="armory-inventory">
   <section className="classic-frame backpack-panel" aria-label="背包">
    ...
    <div className="bag-footer"><span>炉石选中后使用 · 其他消耗品点击使用 · 装备点击查看详情</span><span className="coin-wallet" aria-label={money(s.money)}>{[[Math.floor(s.money/10000),'gold'],[Math.floor(s.money%10000/100),'silver'],[s.money%100,'copper']].map(([count,tone])=><span key={tone}>{count}<i className={'coin '+tone}/></span>)}</span></div>
   </section>
