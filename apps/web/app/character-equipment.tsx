...
import {Bar, GameProps, Icon, ItemDisplay, money} from './game-ui';
...
function ItemSlot({instance,item,label,glyph:Glyph=Backpack,compact=false,selected,onSelect,use,busy,onUse}:{instance?:Instance;item?:ItemData;label:string;glyph?:typeof Shirt;compact?:boolean;selected?:boolean;onSelect?:(uid:string)=>void;use?:{canUse:boolean;reason:string;label:string;description:string};busy?:boolean;onUse?:(uid:string)=>void}){
 ...
 return <ItemDisplay item={item} instance={instance} iconOnly={compact} details={instance.id===6948?'点击选中，再点击使用炉石':use?(use.reason||`点击${use.label} · ${use.description}`):'点击选择物品操作'} trigger={<button type="button" className={'armory-slot filled-slot'+(selected?' selected':'')+(use?' usable-slot':'')} aria-disabled={use&&instance.id!==6948?busy||!use.canUse:undefined} aria-label={`${label}：${item.name}${instance.count>1?' ×'+instance.count:''}`} aria-pressed={!!selected} onClick={()=>{onSelect?.(instance.uid);if(instance.id!==6948&&use?.canUse&&!busy)onUse?.(instance.uid);}}/>}/>;
}
...
export default function CharacterEquipment({state:s,data:d,busy,send,roster}:GameProps){
 const [selected,setSelected]=useState<string|null>(null),[scroll,setScroll]=useState('');
 const enrich=(i:Instance)=>({...i,enchantDescription:d.enchants?.[i.enchant||'']?.description});
 const equipment=Object.fromEntries(Object.entries(s.equipment as Record<number,Instance>).map(([slot,i])=>[slot,enrich(i)]));
 const bag=(s.bag as Instance[]).map(enrich),bags=(s.bags as Instance[]).map(enrich),pending=(s.pending as Instance[]).map(enrich);
 ...
 return <div className="armory-layout">
  <section className="classic-frame character-sheet" aria-label="角色装备">
   ...
   <details className="armory-attributes"><summary>属性与抗性 <small>基本属性 · 攻击 · 防御</small></summary><div className="armory-stat-columns" aria-label="角色属性">{d.characterAttributes.map((group:{title:string;rows:{label:string;value:string}[];note:string})=><section key={group.title} aria-label={group.title}><h3>{group.title}</h3><dl>{group.rows.map(row=><div key={row.label}><dt>{row.label}</dt><dd>{row.value}</dd></div>)}</dl>{group.note&&<p className="armory-stat-note">{group.note}</p>}</section>)}</div></details>
  </section>
  <div className="armory-inventory">
   <section className="classic-frame backpack-panel" aria-label="背包">
    ...
    <div className="bag-bar"><span className="bag-container default-backpack" title="行囊 · 16 格"><Backpack size={25}/><small>16</small></span>{Array.from({length:4},(_,index)=>{const instance=bags[index],item=instance?d.items[instance.id]:undefined;return <span key={index} className={'bag-container '+(!item?'empty-container':'')} title={item?undefined:'背包栏位 '+(index+1)+' · 未装备'} aria-label={item?`${item.name} · ${item.bagSlots} 格`:'空背包栏位 '+(index+1)}>{item?<><ItemDisplay item={item} instance={instance} size={36} iconOnly/><small>{item.bagSlots}</small></>:<Backpack size={23} strokeWidth={1}/>}</span>})}<span className="bag-free">剩余 <b>{Math.max(0,d.bagCapacity-bag.length)}</b> 格</span></div>
    <div className="inventory-tools"><button className="classic-button secondary" disabled={!canAct} onClick={()=>send({type:'sortBag'})}>一键整理</button><button className="classic-button secondary" disabled={!canAct||!discardableJunk.length} onClick={()=>send({type:'discardJunk'})}>一键丢弃灰色物品（{discardableJunk.length}）</button><button className="classic-button secondary" disabled={!canAct||!d.shop.length||!discardableJunk.some(i=>d.items[i.id]?.sell>0)} onClick={()=>send({type:'sellJunk'})}>一键售卖灰色垃圾</button><button className="classic-button secondary" disabled={!canAct||!d.disenchantable?.length} onClick={()=>send({type:'disenchantAll'})}>一键分解（{d.disenchantable?.length||0}）</button></div><div className="bag-grid" aria-label="背包物品格">{Array.from({length:d.bagCapacity},(_,index)=>{const instance=bag[index];return <ItemSlot compact key={instance?.uid||'empty-'+index} instance={instance} item={instance?d.items[instance.id]:undefined} label={'背包第 '+(index+1)+' 格'} selected={selected===instance?.uid} onSelect={setSelected} use={d.itemUses?.[instance?.uid||'']} busy={busy} onUse={uid=>send({type:'useItem',uid})}/>})}</div>
    <div className="bag-footer"><span>炉石选中后使用 · 其他消耗品点击使用 · 悬停查看详情 · 点击选择操作</span><span className="coin-wallet" aria-label={money(s.money)}>{[[Math.floor(s.money/10000),'gold'],[Math.floor(s.money%10000/100),'silver'],[s.money%100,'copper']].map(([count,tone])=><span key={tone}>{count}<i className={'coin '+tone}/></span>)}</span></div>
   </section>
   <ItemTransfer key={s.id} state={s} data={d} busy={busy} send={send} roster={roster}/>
   <section className={'classic-frame selected-item'+(chosen&&chosenItem?' has-selection':'')} aria-label="物品操作" aria-live="polite">
    {chosen&&chosenItem?<><div className="selected-item-heading"><ItemDisplay item={chosenItem} instance={chosen} details={d.itemUses?.[chosen.uid]?.description}/><span>{equipped?'已装备':inBag?'背包物品':'待拾取'}</span><button className="item-detail-close" aria-label="关闭物品操作" onClick={()=>setSelected(null)}>×</button></div>{inBag&&chosenItem.slot>0&&!chosenItem.bagSlots&&equipAction?.equipBlockedReason&&<p className="item-broken">{equipAction.equipBlockedReason}</p>}{inBag&&chosen.id===6948&&<Hearthstone state={s} data={d} busy={busy} send={send}/>}{inBag&&chosen.id!==6948&&d.itemUses?.[chosen.uid]?.reason&&<span className="item-use-note">{d.itemUses[chosen.uid].reason}</span>}{inBag&&<div className="item-actions">{chosen.id!==6948&&d.itemUses?.[chosen.uid]&&<button className="classic-button" disabled={busy||!d.itemUses[chosen.uid].canUse} title={d.itemUses[chosen.uid].reason||undefined} onClick={()=>send({type:'useItem',uid:chosen.uid})}>{d.itemUses[chosen.uid].label}</button>}<button className="classic-button secondary" disabled={!canAct} onClick={()=>send({type:'lockItem',uid:chosen.uid})}>{chosen.locked?'解锁物品':'锁定物品'}</button>{d.inventoryActions?.[chosen.uid]?.tradable&&<button className="classic-button secondary" disabled={!canAct||!!s.dungeon} onClick={()=>send({type:'auctionSell',uid:chosen.uid})}>拍卖快捷上架 · 净得 {money(Math.floor(d.inventoryActions[chosen.uid].quote.sell*chosen.count*.95))}</button>}{d.disenchantable?.includes(chosen.uid)&&<button className="classic-button secondary" disabled={!canAct} onClick={()=>send({type:'disenchant',uid:chosen.uid})}>分解</button>}{!!d.bandages?.[chosen.id]&&<button className="classic-button secondary" disabled={!canAct||!!chosen.locked} onClick={()=>send({type:'useBandage',id:chosen.id})}>使用绷带</button>}{chosenItem.slot>0&&<button className="classic-button" disabled={!canAct||(!chosenItem.bagSlots&&!equipAction?.equippable)||(chosenItem.bagSlots>0&&bags.length>=4&&bags.every(i=>d.items[i.id].bagSlots>=chosenItem.bagSlots))} onClick={()=>send({type:chosenItem.bagSlots?'equipBag':'equip',uid:chosen.uid})}>{chosenItem.bagSlots?(bags.length>=4?'替换最小背包':'装备背包'):'装备'}</button>}{chosenItem.class===2&&[13,22].includes(chosenItem.slot)&&s.learned.includes(674)&&<button className="classic-button" disabled={!canAct||!equipAction?.equippable||d.items[equipment[16]?.id]?.slot===17} onClick={()=>send({type:'equip',uid:chosen.uid,slot:17})}>装备副手</button>}{d.itemUses?.[chosen.uid]?.label==='涂抹主手'&&s.equipment[17]&&<button className="classic-button" disabled={!canAct} onClick={()=>send({type:'useItem',uid:chosen.uid,slot:17})}>涂抹副手</button>}<button className="classic-button secondary" disabled={!canAct||!!d.inventoryActions?.[chosen.uid]?.discardBlockedReason} title={d.inventoryActions?.[chosen.uid]?.discardBlockedReason||undefined} onClick={()=>{send({type:'discardItem',uid:chosen.uid});setSelected(null)}}>丢弃{chosen.count>1?'整组':''}</button>{d.shop.length>0&&chosenItem.sell>0&&<button className="classic-button secondary" disabled={!canAct||!!s.dungeon||d.inventoryActions?.[chosen.uid]?.protected} onClick={()=>send({type:'sell',uid:chosen.uid})}>出售{chosen.count>1?'整组':''} · {money(chosenItem.sell*chosen.count)}</button>}</div>}{(equipped||inBag)&&chosenItem.slot>0&&<div className="enchant-controls"><GameSelect aria-label="选择附魔羊皮纸" value={scroll} onValueChange={nextValue=>setScroll(nextValue)}><GameSelectOption value="">选择适用的附魔羊皮纸</GameSelectOption>{bag.filter(i=>!i.locked&&d.items[i.id]?.enchant&&enchantFits(d.enchants[d.items[i.id].enchant],{class:chosenItem.class,subclass:chosenItem.subclass,InventoryType:chosenItem.slot},({20:5,17:16,13:16,21:16,22:17,23:17,14:17,15:18,25:18,26:18,16:15} as Record<number,number>)[chosenItem.slot]||chosenItem.slot)).map(i=><GameSelectOption key={i.uid} value={i.id}><ItemDisplay item={d.items[i.id]} instance={i} size={28} focusable={false}/></GameSelectOption>)}</GameSelect><button className="classic-button secondary" disabled={!canAct||!scroll||!!chosen.locked} onClick={()=>send({type:'applyEnchant',id:Number(scroll),uid:chosen.uid})}>应用附魔（替换已有附魔）</button></div>}</>:<div className="item-selection-hint"><Sword size={27} strokeWidth={1}/><h3>装备与物品</h3><p>悬停物品查看详情，点击选择装备、使用或出售等操作。</p></div>}
   </section>
   {pending.length>0&&<section className="classic-frame pending-loot"><header className="classic-title"><h3>待拾取战利品</h3><span>{pending.length} 件</span></header><div className="bag-grid">{pending.map(instance=><ItemSlot compact key={instance.uid} instance={instance} item={d.items[instance.id]} label="待拾取" selected={selected===instance.uid} onSelect={setSelected}/>)}</div><div className="pending-actions"><p>腾出背包空间后领取，战利品不会被自动丢弃。</p><button className="classic-button" disabled={!canAct} onClick={()=>send({type:'loot'})}>拾取战利品</button></div></section>}
