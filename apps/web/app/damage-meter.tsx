...
const classes:Record<number,{color:string;name:string;icon?:string}>={
 1:{color:'#c69b6d',name:'战',icon:'class-assets/ability_warrior_offensivestance.jpg'},2:{color:'#f48cba',name:'骑',icon:'class-assets/spell_holy_divineintervention.jpg'},
 3:{color:'#aad372',name:'猎',icon:'class-assets/ability_hunter_beasttaming.jpg'},
 4:{color:'#fff468',name:'贼',icon:'class-assets/ability_stealth.jpg'},
 5:{color:'#ffffff',name:'牧',icon:'class-assets/spell_holy_wordfortitude.jpg'},7:{color:'#0070dd',name:'萨',icon:'class-assets/spell_nature_lightning.jpg'},
 8:{color:'#3fc7eb',name:'法',icon:'class-assets/spell_frost_frostbolt02.jpg'},9:{color:'#8788ee',name:'术',icon:'class-assets/spell_shadow_shadowbolt.jpg'},11:{color:'#ff7c0a',name:'德',icon:'class-assets/ability_druid_catform.jpg'},
};
export function damageSourceIcon(spell:any,skills:any[]){
 const skill=skills.find((candidate:any)=>Number(candidate.spellId)===Number(spell.spellId));
 if(skill?.icon)return skill.icon;
 if(Number(spell.spellId)===75||/自动射击/.test(spell.label))return '/icons/class-assets/ability_whirlwind.jpg';
 if(/宠物|撕咬|爪击|火焰箭/.test(spell.label))return '/icons/class-assets/ability_hunter_pet_wolf.jpg';
 if(/近战|副手|额外攻击|攻击/.test(spell.label))return '/icons/class-assets/ability_meleedamage.jpg';
 return '/icons/class-assets/spell_holy_magicalsentry.jpg';
}
...
export default function DamageMeter({battle,dungeon,clock,skills=[]}:{battle:any;dungeon:any;clock:number;skills?:any[]}){
 ...
 return <section className="damage-meter" aria-label="伤害统计">
  ...
  {!collapsed&&<>
   <div className="dm-toolbar"><GameSelect aria-label="统计战斗范围" value={overall?'dungeon':'encounter'} onValueChange={nextValue=>{setScope(nextValue);setExpanded(null);}}><GameSelectOption value="encounter">{battle.endedAt!=null?'上一场战斗':'当前战斗'}</GameSelectOption><GameSelectOption value="dungeon" disabled={!dungeon?.metrics}>副本已结算战斗</GameSelectOption></GameSelect><span className="dm-timer" title="有效战斗时长"><Clock3 size={11} aria-hidden="true"/>{timer(elapsed)}</span><span className={`dm-state ${battle.endedAt==null&&!overall?'live':''}`}>{overall?'累计':battle.endedAt!=null?'已结束':'战斗中'}</span></div>
   ...
   <div className="dm-list">{!rows.length&&<p className="dm-empty">等待战斗数据…</p>}{rows.map((row:any,index:number)=>{
    ...
    return <div className="dm-entry" key={row.actorId} style={{'--dm-color':identity.color} as CSSProperties}>
     <button className={`dm-row ${selected?'selected':''}`} aria-expanded={selected} aria-label={`${row.name}，伤害 ${row.damage}，DPS ${row.dps.toFixed(1)}，占比 ${(row.share*100).toFixed(1)}%${row.petDamage?`，其中宠物 ${row.petDamage}`:''}，技能明细`} onClick={()=>setExpanded(selected?null:row.actorId)}>
     ...
     {selected&&<div className="dm-breakdown"><div className="dm-breakdown-title"><span>{row.name} · 技能伤害{row.petDamage?`（宠物 ${number(row.petDamage)}）`:''}</span><small>伤害 / 占比</small></div>{!row.spells.some((spell:any)=>spell.damage>0)&&<p className="dm-empty">尚未造成有效伤害</p>}{row.spells.filter((spell:any)=>spell.damage>0).map((spell:any)=>{
      return <div className="dm-spell" key={spell.key}><i style={{width:`${spell.share*100}%`}} aria-hidden="true"/><div className="dm-spell-main"><Icon src={damageSourceIcon(spell,skills)} name={spell.label} size={18}/><span>{spell.label}</span><b>{number(spell.damage)} <small>{(spell.share*100).toFixed(1)}%</small></b></div><div className="dm-spell-meta">{spell.hits} 次命中 · {spell.crits} 次暴击{spell.periodicDamage>0?` · 周期伤害 ${number(spell.periodicDamage)}`:''}</div></div>;
     })}</div>}
    </div>;
   })}</div>
