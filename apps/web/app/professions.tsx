...
export default function Professions({state:s,data:baseData,busy,revision,send}:GameProps){
 ...
 return <div className="economy-layout">
  ...
  <div className="economy-main"><section className="panel"><div className="section-heading"><div><div className="eyebrow">{p.kind}</div><h2>{p.name}</h2></div>{p.learned&&<span className="skill-number">{p.skill} / {p.cap}</span>}</div><p>{p.description}</p><TrainerNavigation state={s} data={d} busy={busy} send={send} kind="profession"/>
