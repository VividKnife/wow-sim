  ...
  {p.recipeCount>0?<section className="panel"><div className="section-heading"><h2>配方制造 <small>{p.recipeCount} 条</small></h2><input aria-label="搜索配方" placeholder="中文、英文或配方编号…" value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}}/></div>
   ...
   {workshopError&&<p className="error" role="alert">{workshopError}</p>}<div className="recipe-list">{recipes.map(r=>{
    ...
    return <article className={'recipe-card '+(!r.known?'recipe-locked':'')} key={r.id}>
     <div className="recipe-title"><Icon src={d.items[r.item]?.icon} name={r.name}/><div><h3>{r.name}</h3><small>需要 {p.name} {r.skill} · 产出 ×{r.output*(valid?count:1)}{r.outputMax>r.output?'—'+r.outputMax*(valid?count:1):''} <span className={'skill-color skill-'+r.color}>{colorNames[r.color]}</span></small></div></div>
     ...
     {d.items[r.item]?.description&&<p className="enchant-text">{d.items[r.item].description}</p>}
     <div className="recipe-materials">{r.materials.map(m=><span className={m.have<m.count*count?'material-missing':''} key={m.id}>{d.items[m.id]?.name} <b>{m.have} / {m.count*(valid?count:1)}</b>{m.bank>0&&<small>银行 {m.bank}</small>}</span>)}</div>
