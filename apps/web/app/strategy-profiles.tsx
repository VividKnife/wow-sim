...
export function StrategyProfiles({memberId,profiles,config,busy,send,onLoad}:{memberId:string;profiles:Profile[];config:Config;busy:boolean;send:(command:any)=>Promise<boolean>;onLoad:(config:Config)=>void}){
 ...
 return <section className="strategy-profiles" aria-label="已保存的策略方案">
  ...
  {profiles.length>0?<>
   <div className="strategy-profile-actions"><GameSelect aria-label="已保存策略方案" value={selected?.name||''} disabled={locked} onValueChange={nextValue=>{setSelectedName(nextValue);setNotice('');}}><GameSelectOption value="">选择已保存的方案</GameSelectOption>{profiles.map(p=><GameSelectOption key={p.name} value={p.name}>{p.name} · {p.totalRules} 条规则</GameSelectOption>)}</GameSelect><Button disabled={locked||!selected} onClick={()=>run('loadProfile')}>读取并应用</Button></div>
