...
export default function Saves(){
 ...
 return <main className={styles.shell} style={sceneStyle}>
  ...
  <form className={styles.creator} onChange={()=>setRequestId('')} onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');const id=requestId||crypto.randomUUID();setRequestId(id);try{const response=await fetch('/api/saves',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:name.trim(),classId,raceId,gender,boost,raidReady:raidMode,requestId:id})});const data=await response.json();if(!response.ok)throw new Error(data.error||'创建失败');window.location.assign(`/?saveId=${encodeURIComponent(data.id)}`);}catch(e){setError(e instanceof Error?e.message:'创建失败，请重试');setBusy(false);}}}>
   ...
   {raidMode&&<div className={styles.boostCard}><p>60级人类法师 · 100金币 · 蓝装核心五人队 · 熔火之心25人远征，可从地下城进入</p></div>}
