...
type Save={id:string;name:string;classId:number;raceId:number;gender:Gender;level:number;location:string;lastSeenAt:number|null};
...
export default function Saves(){
 ...
 const [name,setName]=useState(''),[classId,setClassId]=useState(8),[raceId,setRaceId]=useState(1),[gender,setGender]=useState<Gender>('male'),[boost,setBoost]=useState(false),[raidReady,setRaidReady]=useState(false);
 const raidMode=raidReady&&classId===8&&raceId===1;
 ...
 const chooseRace=(id:number)=>{setRaceId(id);if(id!==1)setRaidReady(false);if(!classOptions.find(c=>c.id===classId)?.races.includes(id))setClassId(classesForRace(id)[0].id);setRequestId('');};
 ...
 return <main className={styles.shell} style={sceneStyle}>
  ...
  <form className={styles.creator} onChange={()=>setRequestId('')} onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');const id=requestId||crypto.randomUUID();setRequestId(id);try{const response=await fetch('/api/saves',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:name.trim(),classId,raceId,gender,boost,raidReady:raidMode,requestId:id})});const data=await response.json();if(!response.ok)throw new Error(data.error||'创建失败');window.location.assign(`/?saveId=${encodeURIComponent(data.id)}`);}catch(e){setError(e instanceof Error?e.message:'创建失败，请重试');setBusy(false);}}}>
   ...
   <section className={styles.hero}><div className={styles.modelStage}><CreationModel key={`${raceId}:${classId}:${gender}:${boost}:${raidMode}`} raceId={raceId} classId={classId} gender={gender} boost={boost} raidReady={raidMode} title={`${selectedRace.name}${gender==='male'?'男性':'女性'}${selectedClass.name} 3D 创建预览`} fallback={<div className={styles.heroPortrait}><AtlasIcon kind="race" position={racePortraits[raceId]}/></div>}/></div><div className={styles.genderChoice} role="group" aria-label="选择性别"><button type="button" disabled={busy||loading} aria-pressed={gender==='male'} className={gender==='male'?styles.active:''} onClick={()=>{setGender('male');setRequestId('');}}><span aria-hidden="true">♂</span>男</button><button type="button" disabled={busy||loading} aria-pressed={gender==='female'} className={gender==='female'?styles.active:''} onClick={()=>{setGender('female');setRequestId('');}}><span aria-hidden="true">♀</span>女</button></div><div className={styles.heroTitle}><small>{selectedRace.nameEn} · {selectedClass.nameEn}</small><h1>{selectedRace.name} {selectedClass.name}</h1><p>{raceDetails[raceId].summary}</p></div></section>
   <fieldset className={styles.classPanel} disabled={busy||loading}><legend><small>选择</small>职业</legend><div className={styles.optionGrid}>{classOptions.map(option=>{const available=availableClasses.some(c=>c.id===option.id);return <button type="button" key={option.id} disabled={!available} className={option.id===classId?styles.selected:''} aria-pressed={option.id===classId} title={available?option.name:'该种族无法选择此职业'} onClick={()=>{setClassId(option.id);if(option.id!==8)setRaidReady(false);setRequestId('');}}><AtlasIcon kind="class" position={classPortraits[option.id]}/><span>{option.name}</span></button>})}</div><div className={styles.classDescription}><strong>{selectedClass.name}</strong><span>{classDetails[classId].role}</span><p>{classDetails[classId].summary}</p></div></fieldset>
   <aside className={styles.descriptionPanel}><h2>{selectedRace.name}</h2><p>{raceDetails[raceId].summary}</p><section className={styles.racialTraits} aria-label={`${selectedRace.name}种族技能`}><h3>种族特长</h3><ul>{racialTraits({raceId}).map((trait:{name:string;description:string})=><li key={trait.name}><strong>{trait.name}</strong><p>{trait.description}</p></li>)}</ul></section><h3>{selectedClass.name}</h3><p>{classDetails[classId].summary}</p><dl><dt>阵营</dt><dd>{selectedRace.faction==='Alliance'?'联盟':'部落'}</dd><dt>性别</dt><dd>{gender==='male'?'男':'女'}</dd><dt>战斗职责</dt><dd>{classDetails[classId].role}</dd><dt>起始地点</dt><dd>{raidMode?'闪金镇':boost?capitals.find(c=>c.id===racialHomes[raceId as keyof typeof racialHomes].capital)?.name:racialHomes[raceId as keyof typeof racialHomes].name}</dd></dl><p className={styles.routeNote}>各族从自己的家园启程，沿联盟或部落路线探索两块大陆。</p><div className={styles.startChoice} role="group" aria-label="起始等级"><button type="button" disabled={busy||loading} aria-pressed={!boost&&!raidMode} className={!boost&&!raidMode?styles.active:''} onClick={()=>{setBoost(false);setRaidReady(false);setRequestId('');}}><b>等级 1</b><small>经典旅程</small></button><button type="button" disabled={busy||loading} aria-pressed={boost} className={boost?styles.active:''} onClick={()=>{setBoost(true);setRaidReady(false);setRequestId('');}}><b>等级 20</b><small>额外 50G</small></button><button type="button" disabled={busy||loading} aria-pressed={raidMode} className={raidMode?styles.active:''} onClick={()=>{setBoost(false);setRaidReady(true);setClassId(8);setRaceId(1);setRequestId('');}}><b>等级 60</b><small>团本体验 · 法师</small></button></div></aside>
   ...
   {raidMode&&<div className={styles.boostCard}><p>60级人类法师 · 100金币 · 蓝装与职业技能 · 四个 14 格背包 · 旅行棕马与骑术 · 冒险者大厅招募，准备团队副本</p></div>}
   {boost&&<div className={styles.boostCard}><div className={styles.coin}><span className={styles.coinIcon}/><strong>额外 50G</strong></div><p>20 级训练师及职业解锁技能 · 本职业任务装备 · 四个 14 格符文布背包 · 旅行棕马与骑术</p></div>}
  </form>
