...
export default function CityServicePanel({service,...props}:GameProps&{service:CityService}){
 ...
 if(service.id==='inn')return <section className="city-service-body city-inn">{s.location==='stormwind'&&s.quests[900001]&&<div><h3>同路人 · 队伍解锁</h3><p>奥里森：旅途漫长，找几位可靠的伙伴同行吧。我可以为你介绍四位冒险者。</p><Button disabled={locked} onClick={()=>send({type:'turnin',id:900001})}>与旅店老板交谈 · 解锁队伍</Button></div>}<div><h3>在这里安家</h3><p>当前炉石绑定：{d.hearthstone.destinationName}。{d.hearthstone.remaining>0?`冷却剩余 ${duration(d.hearthstone.remaining)}。`:'炉石已经就绪。'}重新绑定不会重置冷却。</p><Button disabled={locked||!d.hearthstone.canBind} onClick={()=>send({type:'bindHearth'})}>{s.hearth===s.location&&d.hearthstone.hasItem?'已将这里设为家':'将炉石绑定在这里'}</Button></div><div><h3>在出发前休整</h3><p>按照当前恢复设置使用随身食物与饮水。没有补给时，脱离战斗也会自然恢复生命与法力。</p><Button variant="outline" disabled={locked} onClick={()=>send({type:'rest'})}>使用补给休息</Button></div></section>;
 if(service.id==='flight')return <section className="city-service-body"><div className="city-service-toolbar"><div><h3>飞行管理员</h3><p>{d.flight?.discovered?`你已经掌握了${d.location.name}的飞行路线。`:`与管理员交谈，发现${d.location.name}飞行点。`}</p></div><Button disabled={locked||d.flight?.discovered} onClick={()=>send({type:'unlockFlight'})}>{d.flight?.discovered?'飞行点已发现':'发现飞行点'}</Button></div>{d.flight?.routes.map((f:any)=><div className="city-stock-row" key={f.to}><div className="grow"><strong>{f.name}</strong><small>{duration(f.duration)} · {money(f.cost)}{!f.unlocked?' · 需要先发现两端飞行点':''}</small></div><Button variant="outline" disabled={locked||!f.unlocked||s.money<f.cost} onClick={()=>send({type:'fly',to:f.to})}>乘坐狮鹫</Button></div>)}</section>;
