...
export default function CityServicePanel({service,...props}:GameProps&{service:CityService}){
 ...
 if(service.id==='inn')return <section className="city-service-body city-inn"><div><h3>把暴风城当作归处</h3><p>当前炉石绑定：{d.hearthstone.destinationName}。{d.hearthstone.remaining>0?`冷却剩余 ${duration(d.hearthstone.remaining)}。`:'炉石已经就绪。'}重新绑定不会重置冷却。</p><Button disabled={locked||!d.hearthstone.canBind} onClick={()=>send({type:'bindHearth'})}>{s.hearth===s.location&&d.hearthstone.hasItem?'已将这里设为家':'将炉石绑定在这里'}</Button></div><div><h3>在出发前休整</h3><p>按照当前恢复设置使用随身食物与饮水。没有补给时，请先拜访商人。</p><Button variant="outline" disabled={locked} onClick={()=>send({type:'rest'})}>使用补给休息</Button></div></section>;
