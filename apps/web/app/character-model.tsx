...
export default function CharacterModel({equipment,items,raceId,classId,gender='male',fallback,title='角色 3D 换装预览'}:{equipment:Record<string,{id:number}>;items:Record<string,{slot:number}>;raceId:number;classId:number;gender?:'male'|'female';fallback:ReactNode;title?:string}){
 ...
 return <div ref={container} className={'character-model '+(ready?'model-ready':'')}>
  ...
  {visible&&status!=='error'&&<iframe ref={frame} key={revision+attempt} src="/model-viewer/index.html" title={title} className="model-frame" onLoad={()=>{setState({revision,status:'loading'});frame.current?.contentWindow?.postMessage({channel:'wow-character-equipment',revision,...JSON.parse(revision)},window.location.origin);}}/>}
