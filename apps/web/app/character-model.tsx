...
export default function CharacterModel({equipment,items,fallback,title='人类角色 3D 换装预览'}:{equipment:Record<string,{id:number}>;items:Record<string,{slot:number}>;fallback:ReactNode;title?:string}){
 ...
 const revision=JSON.stringify(modelEquipment(equipment,items));
 ...
 useEffect(()=>{
  ...
  const publish=()=>frame.current?.contentWindow?.postMessage({channel:'wow-character-equipment',revision,items:JSON.parse(revision)},window.location.origin);
  const receive=(event:MessageEvent)=>{
   ...
   if(event.data.status==='ready'){publish();return;}
 ...
 return <div ref={container} className={'character-model '+(ready?'model-ready':'')}>
  {!ready&&<div className="model-fallback" aria-hidden="true">{fallback}</div>}
  {visible&&status!=='error'&&<iframe ref={frame} key={revision+attempt} src="/model-viewer/index.html" title={title} className="model-frame" onLoad={()=>{setState({revision,status:'loading'});frame.current?.contentWindow?.postMessage({channel:'wow-character-equipment',revision,items:JSON.parse(revision)},window.location.origin);}}/>}
  <div className="model-status" role="status">
   {!visible?'人物预览已暂停':status==='loading'?'正在加载 3D 外观…':status==='error'?<>3D 外观暂不可用 <button onClick={()=>{setState({revision,status:'loading'});setAttempt(n=>n+1);}}>重试</button></>:status==='partial'?'部分装备外观暂不可用':'拖动人物旋转'}
  </div>
  {ready&&<a className="model-credit" href="https://www.wowhead.com/classic/dressing-room" target="_blank" rel="noreferrer">模型查看器 · Wowhead</a>}
 </div>;
}
