...
import {BookOpen,Map as MapIcon,Castle,UserRound,UsersRound,Swords,Flag} from 'lucide-react';
...
export default function Game(){
 ...
 useEffect(()=>{const update=()=>localClient.current?.visibility(!document.hidden,battleOpen||activeTab==='arena'||activeTab==='battleground');update();document.addEventListener('visibilitychange',update);return()=>document.removeEventListener('visibilitychange',update);},[battleOpen,activeTab]);
...
<footer className="site-footer"><span>1—60 级 · 九职业经典旅程</span><span>联盟与部落 · 种族家园 · 社区经典数据参考 · 2D 冒险改编</span></footer></div></Tabs>
{s&&signedIn&&!d.goldRaid?.active&&(!game.instance||game.instance.leaderId===s.id)&&<LootWindow key={`${s.id}:${s.lastCombat?.id||'pending'}`} {...props}/>}
{s&&d.ammoPrompt&&<AmmoRestockDialog key={`${d.ammoPrompt.memberId}:${d.ammoPrompt.trigger}`} prompt={d.ammoPrompt} busy={busy} send={send}/>}
