...
export default function Game(){
 ...
 battleVisible.current=battleOpen&&interfaceStyle!=='classic';
 ...
 useEffect(()=>{const update=()=>localClient.current?.visibility(!document.hidden,battleOpen||inlineBattle||interfaceStyle==='classic'||activeTab==='pvp');update();document.addEventListener('visibilitychange',update);return()=>document.removeEventListener('visibilitychange',update);},[battleOpen,inlineBattle,activeTab,interfaceStyle]);
 ...
 const send=useCallback(async(body:any)=>{manualPending.current++;setBusy(true);setError('');try{const characterId=selectedCharacterRef.current;const success=await queue.current!({...body,...(characterId?{characterId}:{}),requestId:crypto.randomUUID()});if(success){setConnectionError('');playQuestSound(body);}if(success&&body.type==='enterDungeon'){const tab=['molten-core','molten-core-gold'].includes(body.contentId)?'raid':'dungeon';setActiveTab(tab);if(classicActive.current)setClassicPanel(tab);}if(success&&classicActive.current&&['hunt','travel','navigateQuest','goldStart','raidStart','dungeonNext','dungeonNavigate','raidNavigate','goldNavigate'].includes(body.type))setClassicPanel(null);if(success&&!classicActive.current&&opensBattleDialog(body,inlineBattleRef.current))setBattleOpen(true);return success;}catch(e:any){if(e.status===401){setSignedIn(false);setConnectionError('');setError('');return false;}if(body.type==='goldBid'&&e.status>=400&&e.status<500){try{const id=selectedCharacterRef.current;await apply(await readGameResponse(await saveFetch(`/api/game?${new URLSearchParams(id?{characterId:id}:{})}`)));}catch{/* Keep the original rejection when refresh is unavailable. */}}setError(gameErrorMessage(e));return false;}finally{manualPending.current--;setBusy(manualPending.current>0);}},[]);
 ...
 const journeyOverview=s&&d?<JourneyActivity {...props} hideTravelProgress={interfaceStyle==='classic'} activityLabel={labels[s.activity.type]||s.activity.type} onObserve={()=>setBattleOpen(true)}/>:null;
...
{error&&<div className="error toast" role="alert">{error}<button aria-label="关闭提示" onClick={()=>setError('')}>×</button></div>}</>;
 ...
 const status=<>{localStatus&&<div className="activity-strip" role="status">{localStatus}</div>}{!signedIn&&<section className="activity-strip" role="alert"><span>登录已过期，请重新登录以继续冒险。</span><a href="/login">重新登录 →</a></section>}{signedIn&&connectionError&&<div className="activity-strip" role="status">{connectionError}</div>}</>;
 const renderClassicPanel=(panel:string)=>{
  if(panel==='map'&&(s.dungeon||d.guildRaid?.active||d.goldRaid?.active))return <DungeonMap {...props} key={d.goldRaid?.active?'gold':d.guildRaid?.active?'guild':s.dungeon?.id} raid={d.goldRaid?.active?'gold':d.guildRaid?.active?'guild':undefined}/>;
  if(['nearby','quests','map','activities'].includes(panel))return <World {...props} surface={panel as 'nearby'|'quests'|'map'|'activities'} onNavigate={()=>setClassicPanel('map')} onOpenDungeon={()=>setClassicPanel('dungeon')} onObserve={()=>setBattleOpen(true)}/>;
  if(['character','bag','mounts'].includes(panel))return <>{game.roster?.length>1&&<CharacterPicker roster={game.roster} value={selectedCharacter||s.id} disabled={busy} onChange={id=>void selectCharacter(id)}/>}<Character key={s.id} {...props} section={characterSection} onSectionChange={setCharacterSection}/></>;
  if(panel==='party')return <Party {...props}/>;
  if(panel==='dungeon')return <DungeonPage {...props} onObserve={()=>setBattleOpen(true)} onOpenParty={()=>setClassicPanel('party')} onConfigure={()=>{setCharacterSection('策略');setClassicPanel('character');}}/>;
  if(panel==='raid')return <RaidPage {...props} onObserve={()=>setBattleOpen(true)}/>;
  if(panel==='pvp')return <Pvp {...props}/>;
