...
export default function Game(){
 ...
 const acceptedResponse=useRef<any>(null),combatPolling=useRef(false);
 combatPolling.current=!!game?.state?.combat&&battleOpen;
 ...
 useEffect(()=>{
  ...
  const poller=createSnapshotPoller({
   delay:()=>combatPolling.current?200:1000,
 ...
 const s=game?.state,d=game?.view,props={state:s,data:d,busy,revision:game?.revision,send};const labels:any={classChannel:'引导职业技能',classSpell:'施放职业技能',mount:'召唤坐骑',escortMove:'跟随迪菲亚叛徒',teleport:'传送中',hearth:'炉石返回中',dungeonCannon:'点燃火炮',resurrect:'复活队友',idle:'等待行动',hunt:s?.combat?'战斗中':s?.rest?'补给恢复':'自动狩猎',travel:'旅行中',conjure:'制造补给',professionGather:'自动采集资源',gather:'调查中',questItem:'使用任务物品',dead:'角色已死亡',revive:'返回尸体'};
 ...
 {!signedIn&&s&&<section className="activity-strip" role="alert"><span>登录已过期，请重新登录以继续冒险。</span><Button asChild><a href="/login">重新登录 →</a></Button></section>}
 ...
 <section className="activity-strip" aria-live="polite"><span className={'activity-light '+s.activity.type}/><div className="grow"><strong>{s.combat?'战斗中':labels[s.activity.type]||s.activity.type}</strong><span> · {d.location.name}{s.activity.to?' → '+d.map.find((n:any)=>n.id===s.activity.to)?.name:''}</span><small>{s.activity.reason|| (s.activity.endsAt?`剩余 ${duration(s.activity.endsAt-s.clock)}`:s.dungeon?'副本战斗在后台继续，每场结束后等待队长继续推进。':'野外活动离线继续；脱离战斗后自动恢复生命与法力。')}</small></div>{s.hp<=0?<><Button onClick={()=>send({type:'revive'})} disabled={busy||!!s.combat}>复活</Button>{d.canSoulstoneRevive&&<Button onClick={()=>send({type:'soulstoneRevive'})} disabled={busy}>灵魂石复活</Button>}{d.reincarnation&&<Button title={d.reincarnation.reason} onClick={()=>send({type:'reincarnate'})} disabled={busy||!d.reincarnation.canUse}>复生</Button>}</>:<Button variant="outline" onClick={()=>send({type:'stop'})} disabled={busy||s.activity.type==='idle'&&!s.rest||s.activity.flight&&s.activity.stopAtNext}>{s.activity.flight?(s.activity.stopAtNext?'已申请下一站停靠':'下一飞行点停靠'):'停止'}</Button>}</section>
 <ActivityProgress state={s} data={d} running/>
 {(s.combat||s.lastCombat)&&<Button className="enter-battle" variant="outline" onClick={()=>setBattleOpen(true)}>进入战斗界面 ⚔</Button>}{(s.combat||s.lastCombat)&&<Battle {...props} canLead={!game.instance||game.instance.leaderId===s.id} open={battleOpen} onOpenChange={setBattleOpen}/>}{s.combat&&<section className="combat-strip"><div className="eyebrow">实时战斗</div>{s.combat.enemies.filter((e:any)=>!e.dead).map((e:any)=><div className="enemy-status" key={e.id}><strong>{e.name} <small>Lv.{e.level}</small></strong><Bar label="生命" value={e.hp} max={e.maxHp}/></div>)}{s.cast&&<p>{d.skills.find((a:any)=>a.spellId===s.cast.spell)?.name||'施法'} · {duration(s.cast.until-s.clock)}</p>}</section>}
