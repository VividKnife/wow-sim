...
export default function Game(){
 ...
 useEffect(()=>{const update=()=>localClient.current?.visibility(!document.hidden,battleOpen||activeTab==='arena');update();document.addEventListener('visibilitychange',update);return()=>document.removeEventListener('visibilitychange',update);},[battleOpen,activeTab]);
 ...
 useEffect(()=>{if(game?.view?.goldRaid?.phase==='auction')setBattleOpen(false);},[game?.view?.goldRaid?.phase]);
 const priorRaidActive=useRef(false);
 useEffect(()=>{const active=!!(game?.view?.guildRaid?.active||game?.view?.goldRaid?.active);if(active&&!priorRaidActive.current)setActiveTab('dungeon');priorRaidActive.current=active;},[game?.view?.guildRaid?.active,game?.view?.goldRaid?.active]);
 ...
 const s=game?.state,d=game?.view,props={roster:game?.roster,state:s,data:d,busy,revision:game?.revision,playback:game?.playback,contentVersion:game?.contentVersion,simulationStatus:localStatus,send};const labels:any={arenaPrepare:'竞技场准备',arenaCombat:'竞技场战斗',goldAuction:'金团公开拍卖',goldRecovery:'金团休整',raidRecovery:'公会营地休整',stockadesQuestEvent:'调查暴风城密谋',classChannel:'引导职业技能',classSpell:'施放职业技能',mount:'召唤坐骑',escortMove:'跟随迪菲亚叛徒',teleport:'传送中',hearth:'炉石返回中',dungeonCannon:'点燃火炮',resurrect:'复活队友',idle:'等待行动',hunt:s?.combat?'战斗中':s?.rest?'补给恢复':'自动狩猎',travel:'旅行中',conjure:'制造补给',professionGather:'自动采集资源',gather:'调查中',questItem:'使用任务物品',dead:'角色已死亡',revive:'返回尸体'};
