...
export default function Game(){
 ...
 if(!queue.current)queue.current=createCommandQueue(async(command:any)=>{const response=await saveFetch('/api/game',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(command)});await apply(await readGameResponse(response));return true;});
 ...
 const s=game?.state,d=game?.view,props={roster:game?.roster,state:s,data:d,busy,revision:game?.revision,playback:game?.playback,contentVersion:game?.contentVersion,send};const labels:any={stockadesQuestEvent:'调查暴风城密谋',classChannel:'引导职业技能',classSpell:'施放职业技能',mount:'召唤坐骑',escortMove:'跟随迪菲亚叛徒',teleport:'传送中',hearth:'炉石返回中',dungeonCannon:'点燃火炮',resurrect:'复活队友',idle:'等待行动',hunt:s?.combat?'战斗中':s?.rest?'补给恢复':'自动狩猎',travel:'旅行中',conjure:'制造补给',professionGather:'自动采集资源',gather:'调查中',questItem:'使用任务物品',dead:'角色已死亡',revive:'返回尸体'};
