...
export default function AmmoRestockDialog({prompt,item,busy,send}:{prompt:any;item?:any;busy:boolean;send:(body:any)=>Promise<boolean>}){
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!busy)void answer(false);}}><DialogContent showCloseButton={false}>
  <DialogHeader><DialogTitle>{prompt.name} 的弹药即将耗尽</DialogTitle><DialogDescription>返回城镇时检测到弹药不足。 当前 {prompt.current} 发，低于 {prompt.threshold} 发。</DialogDescription></DialogHeader>
  ...
  <p><ItemDisplay item={item||{name:prompt.itemName}}/> · 每组 {prompt.packSize} 发 · 本次预计 {packs*prompt.packSize} 发，花费 {money(cost)}</p>
