...
export default function WorldMap({state:s,data:d,busy,send}:GameProps){
 ...
 return <section className="panel map-panel" aria-label="区域地图">
  ...
  <div className="region-map-scroll" tabIndex={0} aria-label="地点地图，窄屏可横向滚动"><div className={'region-map '+(!config.image?'schematic-map':'')}>
   ...
   {player&&player.region===region&&<div key={region} className={'map-player'+(moving?' is-moving':'')} style={{left:player.x+'%',top:player.y+'%'}} role="img" aria-label={moving?'玩家位置：旅行中':'玩家当前位置'}><span>▲</span><b>{moving?(s.activity.flight?'飞行中':'行进中'):'你在这里'}</b></div>}
  </div></div>
  <div className="map-legend"><span>▲ 玩家</span><span>数字 / ⚔ 地点</span><span>↗ 鸟点（标注解锁状态）</span><span>虚线：待行进 · 蓝线：已走过</span></div>
