...
export type Save={id:string;name:string;classId:number;raceId:number;gender:'male'|'female';level:number;location:string;lastSeenAt:number|null};
type Appearance={raceId:number;classId:number;gender:'male'|'female';equipment:Record<string,{id:number}>;items:Record<string,{slot:number}>;location:string};
...
function SavedModel({save}:{save:Save}){
 ...
 return <><CharacterModel raceId={appearance.raceId} classId={appearance.classId} gender={appearance.gender} equipment={appearance.equipment} items={appearance.items} title={`${save.name} · 实际装备 3D 预览`} fallback={<div className={styles.loading}>正在召唤 {save.name}…</div>}/><span className={styles.location}>{appearance.location}</span></>;
}
...
export default function CharacterSelection({saves,loading,error,onCreate,onDelete,onReload}:{saves:Save[];loading:boolean;error:string;onCreate:()=>void;onDelete:(save:Save)=>Promise<void>;onReload:()=>void}){
 ...
 return <main className={styles.screen} style={style}>
  ...
  <div className={styles.layout}>
   <section className={styles.stage} aria-label="当前角色预览">{selected?<><div className={styles.identity}><h2>{selected.name}</h2><p>等级 {selected.level} · {race?.name} {cls?.name}</p></div><div className={styles.model}>{selected.lastSeenAt===null?<p role="status">存档状态无效，请删除后重新创建角色。</p>:<SavedModel key={selected.id} save={selected}/>}</div></>:<div className={styles.empty}><h2>{loading?'正在读取角色…':'新的冒险在等待'}</h2><p>{loading?'':error?'请重新读取角色列表。':'创建你的第一个角色，踏入艾泽拉斯。'}</p></div>}</section>
   <aside className={styles.roster}><h2>你的角色 <small>{saves.length} / 20</small></h2><div className={styles.list} role="group" aria-label="选择角色">{saves.map(save=><button key={save.id} disabled={busy} aria-pressed={selected?.id===save.id} className={selected?.id===save.id?styles.selected:''} style={{'--entry-color':colors[save.classId]} as CSSProperties} onClick={()=>{setSelectedId(save.id);setConfirm(false);setDeleteError('');}}><strong>{save.name}</strong><span>等级 {save.level} · {raceOptions.find(r=>r.id===save.raceId)?.name} {classOptions.find(c=>c.id===save.classId)?.name}</span><small>{save.lastSeenAt===null?'需要重新创建':`上次冒险 ${new Date(save.lastSeenAt).toLocaleDateString('zh-CN')}`}</small></button>)}</div><button className={styles.redButton} disabled={busy||loading||saves.length>=20} onClick={onCreate}>创建新角色</button><button className={styles.delete} disabled={busy||!selected} onClick={()=>{setConfirm(true);setDeleteError('');}}>删除角色</button></aside>
  </div>
  {error&&<div className={styles.error} role="alert">{error} <button onClick={onReload}>重新读取</button></div>}
  <footer className={styles.footer}><span>{selected?`${race?.faction==='Alliance'?'联盟':'部落'} · ${saves.length} 个角色`:'每个角色拥有独立冒险进度'}</span><button className={styles.enter} disabled={!selected||selected.lastSeenAt===null||busy||loading} onClick={()=>selected&&window.location.assign(`/?saveId=${encodeURIComponent(selected.id)}`)}>进入游戏</button><span>选择角色，继续你的冒险</span></footer>
