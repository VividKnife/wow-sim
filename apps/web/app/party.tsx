...
import {Shield,Heart,Swords,Plus,ArrowRightLeft,UsersRound,Check} from 'lucide-react';
...
export default function Party(props:GameProps){
 ...
 return <div><nav className="party-mode-tabs" aria-label="队伍玩法"><button type="button" aria-pressed={mode==='owned'} onClick={()=>setMode('owned')}>我的队友</button><button type="button" aria-pressed={mode==='hall'} onClick={()=>setMode('hall')}>副本组队</button></nav>{mode==='hall'?<AdventureHall {...props}/>:<OwnedParty {...props}/>}</div>;
}
function OwnedParty({state:s,data:d,busy,send}:GameProps){
 ...
 return <div className={styles.page}>
  <section className={styles.overview} aria-label="阵容推荐">
   <div className={styles.heading}><div><span className={styles.eyebrow}>五人小队</span><h2>队伍阵容</h2><p>经典五人阵容 · 1 坦克 / 1 治疗 / 3 输出</p></div><span className={styles.size}><UsersRound size={18}/>{s.party.length+1}<small>/ 5</small></span></div>
  ...
  <section className={styles.rosterSection} aria-label="当前队伍"><div className={styles.sectionHeading}><h2>同行伙伴</h2><span>野外同行 · 副本候选</span></div><div className={styles.roster}>
   ...
   {Array.from({length:Math.max(0,4-s.party.length)},(_,index)=><a href="#party-recruit" key={index} className={styles.emptySeat}><Plus size={24}/><strong>等待新伙伴</strong><small>选择职业与职责 →</small></a>)}
  </div></section>
  {canManage&&<section id="party-recruit" tabIndex={-1} className={styles.recruitSection} aria-label="招募与更换"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>职业与职责</span><h2>{replacing?`接替 ${replacement.name}`:'招募伙伴'}</h2></div><span>{replacing?'更换 · 10金币':'新增队友 · 免费'}</span></div>
   ...
   <div className={styles.candidates}>{shown.map((c:any)=>{const role=chooseRole(c),recommended=c.recommendedRoles.includes(role);return <article key={c.id} className={styles.candidate} data-recommended={recommended}>
    <div className={styles.candidateHeading}><Portrait id={c.classId}/><div><h3>{classes[c.classId]}</h3><p>{c.name} · Lv.{c.level}</p></div>{recommended&&<span className={styles.recommended}><Check size={12}/>补足{partyRoleLabels[groupRole(role)]}</span>}</div>
