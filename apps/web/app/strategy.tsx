...
function MemberStrategy({state:s,data:d,busy,send,member}:GameProps&{member:any}){
 ...
 return <div className="strategy-layout"><section className="panel">
  ...
  <p>从上到下检查条件、已学最高等级、距离、资源、姿态和冷却，优先执行第一条可用规则。每位成员最多 {MAX_STRATEGY_RULES} 条，每条可追加最多 {MAX_STRATEGY_EXTRA_CONDITIONS} 个「并且」条件。可复制同一技能设置不同条件，用序号直接调整优先级。人数按技能实际范围计算；群攻默认保护控场目标。</p>
  ...
  <div className="rule-list">{rules.map((r:any,i:number)=>{return <div className={`rule-editor${r.enabled?'':' strategy-rule-disabled'}`} key={i}>
   <div className="rule-number"><select aria-label={`第 ${i+1} 条优先级`} title="移动到指定优先级" value={i} disabled={busy} onChange={e=>moveTo(i,Number(e.target.value))}>{rules.map((_,n)=><option key={n} value={n}>{n+1}</option>)}</select></div>
