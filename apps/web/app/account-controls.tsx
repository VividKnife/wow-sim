...
export default function AccountControls({game,busy,send}:Props){
 ...
 return <details className="panel account-controls"><summary>账号名册与共享副本</summary>
  ...
  <section><h3>共享实例</h3>{instance?<>
   ...
   <select aria-label="实例内容" value={contentId} onChange={e=>setContentId(e.target.value)}><option value="northshire-skirmish">北郡遭遇</option><option value="deadmines">死亡矿井</option><option value="stockades">暴风城监狱</option></select>
