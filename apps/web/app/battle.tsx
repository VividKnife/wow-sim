...
export default function Battle({state,data,playback,contentVersion,simulationStatus,busy,send,open,onOpenChange,canLead=true,historical=false,embedded=false,commandMemberId,onCommandMemberChange}:{commandMemberId?:string;onCommandMemberChange?:(id:string)=>void;embedded?:boolean;historical?:boolean;open:boolean;onOpenChange:(open:boolean)=>void;canLead?:boolean}&GameProps){
 ...
 const [clock,setClock]=useState(s.clock),[effects,setEffects]=useState<any[]>([]),[lowEffects,setLowEffects]=useLowEffects(),[reducedMotion,setReducedMotion]=useState(false),[selectedId,setSelectedId]=useState(s.id),[zoom,setZoom]=useState(1);
