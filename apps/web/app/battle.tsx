...
import {RaidLiveOrders} from './raid-command';
...
export default function Battle({state,data,playback,contentVersion,simulationStatus,busy,send,open,onOpenChange,canLead=true,historical=false,embedded=false,commandMemberId,onCommandMemberChange}:{commandMemberId?:string;onCommandMemberChange?:(id:string)=>void;embedded?:boolean;historical?:boolean;open:boolean;onOpenChange:(open:boolean)=>void;canLead?:boolean}&GameProps){
 ...
 const pauseCommand=raid?('goldPause'):d.dungeon?.autoAdvance?'dungeonPause':'stop';
 ...
 const viewClock=s.combat?.command?.paused?s.clock:s.combat?Math.min(localLive?s.clock:clock,s.playbackUntil??Infinity):battle.endedAt??clock;
 ...
 const chooseMember=(id:string)=>{setCommandMember(id);setSelectedId(id);setPendingCommand(null);onCommandMemberChange?.(id);};
 const issue=(body:any,targeted=false)=>{
  const enemy=battle.enemies.find((e:any)=>e.id===commandTarget&&e.hp>0&&!e.removed&&!e.controlledBy);
  if(targeted&&!enemy){setPendingCommand(body);return;}
  const {label,...action}=body;void send({type:'combatCommand',encounterId:battle.id,...action,...(targeted?{targetId:enemy.id}:{})});setPendingCommand(null);
 };
 const selectUnit=(id:string)=>{
  setSelectedId(id);
  if(d.combatCommand?.members.some((m:any)=>m.id===id)){chooseMember(id);return;}
  const enemy=battle.enemies.find((e:any)=>e.id===id&&e.hp>0&&!e.removed&&!e.controlledBy);if(!enemy)return;
  setCommandTarget(id);
  if(pendingCommand){const {label,...action}=pendingCommand;void send({type:'combatCommand',encounterId:battle.id,...action,targetId:id});setPendingCommand(null);}
 };
 const commandHud=!historical&&<CombatCommand state={s} data={d} busy={busy} send={send} memberId={commandMemberId||commandMember} targetId={commandTarget} onMemberChange={chooseMember} onTargetChange={selectUnit} onOrder={issue} pending={pendingCommand} onCancel={()=>setPendingCommand(null)} canLead={canLead} active={open} embedded={embedded}/>;
 if(embedded)return <div className="world-embedded-battle"><BattleHD2D scene={scene} skills={skills} onSelect={selectUnit} active={open}/>{commandHud}</div>;


 return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="battle-dialog" showCloseButton={false}><header className="battle-heading"><div><div className="eyebrow">{raid?.name||d.location.name} / {(d.goldRaid?.active)?'25人金团':battle.dungeon?'五人小队':'野外遭遇'}</div><DialogTitle>{s.combat?.command?.paused?'战术暂停':pullCountdown?'准备开怪':opening?'坦克开怪中':s.combat?'战斗进行中':player.hp<=0?'你倒下了':'战斗结束'}</DialogTitle><DialogDescription>3D 战场 · 实时战斗演出 · 选择角色查看目标</DialogDescription></div><Button variant="outline" onClick={()=>onOpenChange(false)}>{(d.goldRaid?.active)?'返回远征营地 ↙':'返回世界 ↙'}</Button></header>{(simulationStatus||playbackStatus)&&<p role="status">{simulationStatus||playbackStatus}</p>}<div className="battle-toolbar"><span>{units.filter(u=>u.foe&&u.hp>0&&!u.removed).length} 个敌人 · {actors.filter(a=>a.hp>0&&!a.totemUnit&&!a.petUnit).length} 名队员</span><div className="battle-zoom" role="group" aria-label="战斗视野缩放"><button type="button" aria-label="缩小战斗视野" disabled={zoom<=.5} onClick={()=>setZoom(z=>Math.max(.5,Math.round((z-.1)*10)/10))}>−</button><output aria-live="polite">{Math.round(zoom*100)}%</output><button type="button" aria-label="放大战斗视野" disabled={zoom>=3} onClick={()=>setZoom(z=>Math.min(3,Math.round((z+.1)*10)/10))}>＋</button><button type="button" aria-pressed={zoom===defaultZoom} onClick={()=>setZoom(defaultZoom)}>默认取景</button></div><label className="battle-effects-control"><input type="checkbox" checked={lowEffects} onChange={e=>setLowEffects(e.target.checked)}/>简化特效</label>{reducedMotion&&<small>减少动态效果</small>}</div><div className="battle-raid-timers">{(d.goldRaid?.active)&&s.combat&&<><strong>{raid?.map?.route.find((b:any)=>b.id===raid.activeBoss)?.name}</strong>{(d.goldRaid).nextMechanics.map((m:any)=><span key={m.name}>{m.name} <b>{duration(Math.max(0,m.at-viewClock))}</b></span>)}</>}</div>{!historical&&canLead&&<RaidLiveOrders state={s} data={d} busy={busy} send={send}/>}<div className="battle-room"><BattleHD2D scene={scene} skills={skills} onSelect={selectUnit} active={open}/>{commandHud}{opening&&!s.combat?.command?.paused&&<div className="battle-pull-countdown" role="status" aria-live="polite" aria-atomic="true">{pullCountdown>0?<><span>准备开怪</span><strong>{pullCountdown}</strong><span>倒计时结束后由坦克先手</span></>:<span>坦克正在开怪</span>}</div>}</div><div className="battle-roster" aria-label="选择敌人或召唤物">{units.filter(u=>u.foe||u.petUnit||u.totemUnit).map(u=><button key={u.id} className={`${selected.id===u.id?'selected':''} ${u.hp<=0?'fallen':''}`} onClick={()=>selectUnit(u.id)}><span style={{color:u.foe?'#df9e89':colors[u.classId]}}>{u.commandMark} {u.name}</span><small>{Math.max(0,Math.round(u.hp/Math.max(1,u.maxHp)*100))}%</small></button>)}</div><div className="battle-unit-detail"><strong>{selected.name}</strong><span>{unitCondition(selected,viewClock)|| (selected.cast?`${castSkill?.name||'施法'}中`:selected.hp<=0?'已倒下':'可行动')}{remaining>0?` · ${duration(remaining)}`:''}</span><span>目标距离 {target?distance(selected,target).toFixed(1)+' 码':'—'}</span><span>移速 {speed.toFixed(1)} 码/秒{slow>0?` · 降低 ${Math.round(slow*100)}%`:''}</span><small>{selected.visual.label} · 圆环 {range} 码</small></div>
 <div className="battle-bottom"><section className="battle-player"><BattleRaidFrames units={units.filter(u=>actors.some(a=>a.id===u.id))} views={projection.units} selectedId={selected.id} onSelect={selectUnit} clock={viewClock} live={!!s.combat}/><details className="battle-member-inspect"><summary>{selected.name} · 效果与职业详情</summary><BattleUnitStatus unit={selected} ui={projection.units[selected.id]} clock={viewClock} live={!!s.combat}/><BattleClassHint classId={selected.classId}/></details><div className="action-meter"><span>{attack.label} <small>{swing.kind==='waiting'?duration(swing.remaining):swingLabels[swing.kind]||swing.kind}</small></span><div role="progressbar" aria-label={attack.label+'进度'} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(swingProgress*100)}><i style={{width:swingProgress*100+'%'}}/></div></div>{playerUi.offhand&&s.combat&&<div className="action-meter"><span>副手攻击 <small>{duration(playerUi.offhand.until-viewClock)}</small></span><div><i style={{width:(['waiting','ready'].includes(meleeStatus(player,s.combat,viewClock).kind)?actionProgress(playerUi.offhand.startedAt,playerUi.offhand.until,viewClock)*100:0)+'%'}}/></div></div>}<BattleCompanions state={s} data={d} busy={busy} send={send} units={units} clock={viewClock} live={!!s.combat} targetId={selected.id}/>
