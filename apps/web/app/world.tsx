...
export default function World({state:s,data:d,busy,revision,send,overview,onOpenDungeon}:WorldProps){
 ...
 if(s.dungeon)return <div className="world-main"><PlayerHud state={s} data={d}/>{overview}<section className="panel"><h2>{s.dungeon?'正在探索'+d.dungeon.name:d.location.region==='暴风城'?'暴风城监狱':'死亡矿井'}</h2><p>在地下城页面查看小队准备、路线进度与遭遇。</p><Button onClick={onOpenDungeon} disabled={!onOpenDungeon}>打开地下城 →</Button></section></div>;
 return <div className={'world-layout '+(d.city?'has-city':' ')}><section className="world-main"><h1 className="journey-title">继续你的旅程</h1><PlayerHud state={s} data={d}/><section className="journey-hero" aria-labelledby="journey-location-title"><div className="journey-hero-art" aria-hidden="true"><span className="journey-sun"/><span className="journey-mountains far"/><span className="journey-mountains near"/><span className="journey-tower"><i/></span><span className="journey-road"/></div><div className="journey-hero-top"><span>{d.location.region} · {d.location.name}</span><span>{d.city?'城镇区域':'野外活动'}</span></div><div className="journey-hero-copy"><div className="eyebrow">THE ROAD AHEAD</div><h2 id="journey-location-title">风吹过{d.location.name}</h2><p>{d.location.region}之外，是通往下一段旅程的路。</p><Button variant="outline" onClick={()=>setMapOpen(!mapOpen)}>{mapOpen?'收起区域地图':'打开区域地图'}　→</Button></div></section>{mapOpen&&<WorldMap state={s} data={d} busy={busy} send={send}/>} {overview}{(d.city||['全部','人物与服务'].includes(filter))&&<LocalNpcs key={'npcs:'+s.id+':'+s.location} state={s} data={d} busy={busy} send={send}/>} {d.city&&<City key={'city:'+s.id+':'+s.location} state={s} data={d} busy={busy} revision={revision} send={send}/>}
 ...
 {(s.location==='deadmines'||s.location==='stockades'||d.location.region==='西部荒野'||d.location.region==='暴风城')&&<section className="panel"><h2>{s.dungeon?'正在探索'+d.dungeon.name:d.location.region==='暴风城'?'暴风城监狱':'死亡矿井'}</h2><p>在地下城页面查看小队准备、路线进度与遭遇。</p><Button onClick={onOpenDungeon} disabled={!onOpenDungeon}>打开地下城 →</Button></section>}
