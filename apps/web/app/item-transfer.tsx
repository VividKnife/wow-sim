...
export default function ItemTransfer({state:s,data:d,busy,send,roster=[]}:GameProps){
 ...
 return <details className="classic-frame item-transfer">
  ...
  <div className="transfer-content">
   ...
   <label className="transfer-recipient">接收角色<select aria-label="接收角色" disabled={busy} value={recipient} onChange={e=>{setRecipient(e.target.value);setNotice('');}}><option value="">请选择接收角色</option>{others.map(c=><option key={c.id} value={c.id}>{c.name} · 背包 {c.bagUsed}/{c.bagCapacity}{c.location!==s.location?' · 不在同一地点':''}</option>)}</select></label>
