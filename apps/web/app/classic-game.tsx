...
export default function ClassicGame(props:Props){
 ...
 const battle=s.combat||s.lastCombat,battling=!!s.combat;
 ...
 const quests=d.quests.filter((q:Model)=>q.active),tracked=quests.find((q:Model)=>q.complete)||quests[0],available=d.quests.filter((q:Model)=>q.canAccept).length;
 ...
 const damageMeter=battle?<DamageMeter battle={battle} dungeon={s.dungeon} clock={s.clock} skills={d.combatSkills||d.skills}/>:<div className="cu-meter-empty"><Swords size={22}/><h3>伤害统计</h3><p>尚无战斗记录。战斗后可查看成员伤害、DPS 与技能明细。</p><button className="cu-gold-button" onClick={()=>open('activities')}>查看野外活动</button></div>;
 return <div className="classic-game">
  <section className={`cu-viewport ${battling?'cu-in-combat':''} ${travelling?'cu-is-travelling':''}`} aria-label="经典游戏主界面">
   ...
   <aside className="cu-left-hud" aria-label="小队状态"><div className="cu-hud-heading"><span>{members.length>5?'团队':'冒险小队'} · {members.length} 人</span><button onClick={()=>open(members.length>5?'raid':'party')}>管理</button></div>{members.length>5?<div className="cu-raid">{members.map((unit:Model)=><button key={unit.id} onClick={()=>open('raid')} style={{'--class-color':colors[unit.classId]||'#a3a084'} as CSSProperties} title={`${unit.name} · ${fmt(unit.hp)} 生命`}><span>{unit.name}</span><i style={{width:percent(unit.hp,d.battleView?.units?.[unit.id]?.maxHp||unit.stats?.maxHp||unit.maxHp)+'%'}}/></button>)}</div>:groupFrames(true)}</aside>
   <aside className="cu-right-hud"><button className="cu-quest-tracker" onClick={()=>open(tracked?'quests':'nearby')}><h2>任务追踪 <span>{quests.length}</span></h2><b>{tracked?`${tracked.complete?'?':'◇'} ${tracked.name}`:available?`! ${available} 个可接任务`:'暂无追踪任务'}</b>{tracked?.objectives.slice(0,3).map((o:Model,i:number)=><p key={i}>{o.name} {o.count}/{o.required}</p>)}<small>{tracked?(tracked.complete?'返回委托人领取报酬':'点击查看任务与导航'):available?'与附近人物交谈':'查看附近人物与服务'}</small></button></aside><div className="cu-corner-meter" ref={meterRef} style={meterStyle}><button className="cu-hud-drag cu-meter-drag" aria-label="移动伤害统计" title="拖动移动 · 方向键微调 · 双击或 Home 复位" {...meterHandle}><GripHorizontal size={16}/></button>{damageMeter}</div>
   ...
   <aside className={`cu-lower-left cu-chat-window ${!chatExpanded?'cu-chat-compact':''}`} aria-label="信息与战报频道" ref={chatRef} style={chatStyle}>
    ...
    <Tabs.Root className="cu-chat-channels" defaultValue="general" onValueChange={()=>setChatExpanded(true)}>
     ...
     <Tabs.Content value="general" className="cu-chat-scroll" tabIndex={0}>
      <div className="cu-chat" role="status"><span>[{instance?'副本':'旅途'}]</span> {busy?'正在处理操作…':s.activity.reason||instanceStatus||s.logs.at(-1)?.text||'选择目的地，开始旅程。'}</div>
      <div className="cu-chat-overview">{overview}</div>
      <div className="cu-channel-history">{s.logs.slice(-30).reverse().map((log:Model)=><p key={log.id}><time>{journeyTime(log.at,s).label}</time><span>{log.text}</span></p>)}</div>
     </Tabs.Content>
