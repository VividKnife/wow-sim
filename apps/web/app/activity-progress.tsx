...
type ProgressState={clock:number;activity:{type:string;startedAt?:number;endsAt?:number;spell?:number;mount?:number;from?:string;to?:string;flight?:boolean};cast?:{startedAt:number;until:number;spell:number}|null;combat?:unknown;rest?:{startedAt?:number;until:number;foodUntil:number;waterUntil:number}|null;presence?:{paused?:boolean}};
...
function Progress({label,start,end,clock,running,journey}:{label:string;start:number;end:number;clock:number;running:boolean;journey?:{from:string;to:string;flight:boolean}}){
