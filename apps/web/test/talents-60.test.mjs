...
import {talents,spells} from '../lib/game/catalog.js';
import * as effects from '../lib/game/talent-effects.js';
...
test('talent events persist deterministic Nightfall and consume it only on Shadow Bolt',async()=>{
 const runtime=await import('../lib/game/talent-runtime.js').catch(()=>({}));
...
test('Swiftmend consumes one owned heal over time and heals its correct duration',async()=>{
 const runtime=await import('../lib/game/talent-runtime.js').catch(()=>({}));assert.equal(typeof runtime.executeTalentActive,'function');
...
test('incoming talent proc does not recursively trigger itself',async()=>{
 const runtime=await import('../lib/game/talent-runtime.js').catch(()=>({}));assert.equal(typeof runtime.onTalentEvent,'function');
...
test('Elemental Warding reduces elemental incoming damage but not shadow',async()=>{
 const {onTalentEvent}=await import('../lib/game/talent-runtime.js');const c=character(7,{'Elemental Warding':3});
...
test('pet Frenzy, Intimidation and owner Spirit Bond produce combat changes',async()=>{
 const runtime=await import('../lib/game/talent-runtime.js');assert.equal(typeof runtime.onPetTalentEvent,'function');
...
test('cast snapshot consumes old proc charges and preserves procs created by this cast',async()=>{
 const runtime=await import('../lib/game/talent-runtime.js');assert.equal(typeof runtime.beginTalentCast,'function');
...
test('Spirit of Redemption delays lethal damage for ten seconds and then expires',async()=>{
 const {onTalentEvent}=await import('../lib/game/talent-runtime.js');const c=character(5,{'Spirit of Redemption':1},{hp:20});const s={clock:0};
...
test('all 27 trees support real level-60 51-point builds including their capstones',async()=>{
 const {classTalentTrees,classDefinitions}=await import('../lib/game/catalog.js');const {createGame,act,view}=await import('../lib/game/engine.js');
...
test('wand and ranged weapon talents do not amplify unrelated spells or melee crit',async()=>{
 const {items}=await import('../lib/game/catalog.js');const wand=Object.values(items).find(i=>i.class===2&&i.subclass===19),bow=Object.values(items).find(i=>i.class===2&&i.subclass===2);assert.ok(wand&&bow);
...
test('coverage ledger explicitly reconciles every talent node to an executor',async()=>{
 const {talentExecutionCoverage}=await import('../lib/game/class-support.js');const missing=Object.values(talents).filter(t=>!talentExecutionCoverage(t).supported).map(t=>`${t.classId}:${t.name}`);assert.deepEqual(missing,[]);
});
test('remaining defensive talents modify real detection and resistance hooks',async()=>{
 const {detectsTarget}=await import('../lib/game/combat-space.js');const observer=character(1,{}, {position:0}),rogue=character(4,{}, {position:4,stealthed:true});assert.equal(detectsTarget(observer,rogue,0),true);rogue.talents=character(4,{'Master of Deception':5}).talents;assert.equal(detectsTarget(observer,rogue,0),false);
