...
test('Berserking scales from ten to thirty percent haste using activation health',async()=>{
 const racial=await import('../../../packages/game-domain/src/rules/racial-effects.js').catch(()=>({}));assert.equal(typeof racial.activateRacial,'function');
...
test('Cannibalize heals in five two-second ticks and damage cancels it',async()=>{
 const racial=await import('../../../packages/game-domain/src/rules/racial-effects.js').catch(()=>({}));assert.equal(typeof racial.tickRacialEffects,'function');
