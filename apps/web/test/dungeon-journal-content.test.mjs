...
test('versioned content carries a browsable journal with display-safe loot and playable states',()=>{
 ...
 assert.deepEqual(journal.filter(d=>d.playable).map(d=>d.id).sort(),['deadmines','stockades']);
...
test('live snapshot includes both playable dungeon views but not the static journal',()=>{
 const s=createGame('手册',5,0),snapshot=projectClientSnapshot(s,view(s));
 assert.deepEqual(Object.keys(snapshot.view.dungeons).sort(),['deadmines','stockades']);
