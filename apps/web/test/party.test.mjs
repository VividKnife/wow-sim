...
test('recruits match current level and bounded equipment; identity and gear persist after player growth',()=>{
 let s=createGame('队长',2,0);s.level=18;const candidates=view(s).candidates;assert.equal(candidates.length,4);
 for(const candidate of candidates)s=act(s,{type:'recruit',id:candidate.id},0);
 assert.equal(s.party.length,4);assert.equal(new Set(s.party.map(c=>c.id)).size,4);
 for(const c of s.party){assert.equal(c.level,18);assert.equal(c.hp,stats(c).maxHp);for(const item of Object.values(c.equipment)){assert.ok(canEquip(c,items[item.id]));assert.ok(items[item.id].ItemLevel<=c.recruitedGearCap);assert.equal(item.issued,true);}}
 ...
 assert.throws(()=>act(s,{type:'recruit',id:candidates[0].id},5000));
});
test('loot allocation moves exactly one real instance and preserves bind ownership',()=>{
 let s=createGame('分配',2,0);s.level=18;s=act(s,{type:'recruit',id:'warrior'},0);addItem(s,1270);const item=s.bag.find(i=>i.id===1270);s=act(s,{type:'equip',uid:item.uid,target:s.party[0].id},0);
...
test('party view exposes learned spell names and original icons without adding trainer offers',()=>{
 let s=createGame('技能展示',2,0);s.level=20;for(const id of ['warrior','priest','rogue'])s=act(s,{type:'recruit',id},0);
...
test('a two-handed weapon removes the offhand and prevents simultaneous shield equipment',()=>{
 let s=createGame('双手武器',2,0);s.level=18;s=act(s,{type:'recruit',id:'warrior'},0);const target=s.party[0].id;assert.ok(s.party[0].equipment[17]);
...
test('equipping a full throwing-weapon stack preserves count and instance identity in ranged slot',()=>{
 let s=createGame('投掷武器',2,0);s.level=18;s=act(s,{type:'recruit',id:'rogue'},0);addItem(s,2947,200);const knives=s.bag.find(i=>i.id===2947);
...
test('two-hand swaps need room for both displaced loot items and preserve their ownership',()=>{
 let s=createGame('装备空间',2,0);s.level=18;s=act(s,{type:'recruit',id:'warrior'},0);const target=s.party[0].id;
 s.party[0].learned.push(227);
