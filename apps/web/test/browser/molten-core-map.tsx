...
function Harness(){
 ...
 const send=async(action:any)=>{try{let s;if(action.type==='leaveInstance'||action.type==='enterDungeon'){s=structuredClone(current.current);if(action.type==='leaveInstance')leaveGuildRaid(s);else enterGuildRaid(s,action.contentId);}else s=act(current.current,action,current.current.wallAt);current.current=s;setState(s);setError('');return true;}catch(e:any){setError(e.message);return false;}};
