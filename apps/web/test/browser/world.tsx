...
function fixture(location='goldshire'){
 const horde=['valley-of-trials','orgrimmar'].includes(location);
 let s:any=createGame('地图测试',11,0,horde?{raceId:2,classId:7}:{});s.level=horde?1:4;if(!horde){s.location='goldshire';s=act(s,{type:'accept',id:62},0);}s.location=location;
...
function Harness(){
 ...
 return <main className="game-shell"><header className="panel" style={{margin:'20px 0'}}><h2>独立地图测试 · 不连接用户存档</h2><div className="action-row">{[['goldshire','艾尔文夹具'],['sentinel','西部荒野夹具'],['stormwind','暴风城夹具'],['ironforge','铁炉堡夹具'],['valley-of-trials','部落出生夹具'],['orgrimmar','奥格瑞玛夹具']].map(([id,name])=><button key={id} onClick={()=>{setState(fixture(id));setError('');}}>{name}</button>)}<button onClick={()=>setState(advance(s,s.wallAt+Math.floor(Math.max(0,(s.activity.endsAt||s.clock)-s.clock)/2)).state)}>前进一半时间</button><button onClick={()=>setState(advance(s,s.wallAt+Math.max(0,(s.activity.endsAt||s.clock)-s.clock)).state)}>抵达目的地</button></div></header><World state={s} data={{...clientContent(),...view(s)}} busy={false} send={send}/>{error&&<p role="alert">{error}</p>}</main>;
}
