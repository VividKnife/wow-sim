...
import {PARTY_QUEST} from '../../../../packages/game-domain/src/rules/party-unlock.js';
...
function fixture(ground='grass',group=true){
 ...
 s.completed[PARTY_QUEST]=true;s.location='stormwind';
...
function Preview(){
 ...
 return <main style={{padding:'24px',maxWidth:1280,margin:'auto'}}><h1>HD-2D 战斗画面</h1><p>真实战斗引擎驱动的独立预览，不连接账号或存档。关闭战斗面板可切换场景。</p><div className="action-row" style={{margin:'18px 0'}}>
  <label>场景 <select aria-label="场景" value={ground} onChange={e=>{setGround(e.target.value);setState(fixture(e.target.value));setRun(n=>n+1);setOpen(true);}}>{Object.entries({grass:'林间旷野',dirt:'暮色荒野',cave:'幽深矿洞',deck:'暮港甲板',water:'碧水浅滩'}).map(([value,name])=><option key={value} value={value}>{name}</option>)}</select></label>
