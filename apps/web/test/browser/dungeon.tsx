...
function Harness(){
 ...
 return <main className="game-shell"><header className="panel" style={{margin:'20px 0'}}><h2>独立测试角色 · 不连接用户存档</h2><p>18 级与补给为测试夹具；每场战斗使用真实引擎。此页面不代表自然升级通关。</p><div className="action-row">{[['entry','手册'],['stockades','监狱冒险'],['map','矿井地图'],['powder','火药箱'],['cannon','火炮'],['recovery','队长倒下'],['outdoor','野外小队'],['escort-fight','Escort battle'],['escort','护送']].map(([id,name])=><button key={id} onClick={()=>{setState(fixture(id));setOpen(false);setError('');key.current=null;}}>{name}夹具</button>)}</div></header><Escort {...props}/>{['lighthouse','sentinel','moonbrook'].includes(s.location)?<Party {...props}/>:<DungeonPage onObserve={()=>{}} {...props} onOpenParty={()=>setError('队伍管理入口已触发')} onConfigure={()=>setError('配置入口已触发')}/>} {(s.combat||s.lastCombat)&&<><button onClick={()=>setOpen(true)}>查看测试战斗</button><Battle {...props} open={open} onOpenChange={setOpen}/></>}{error&&<p role="alert">{error}</p>}</main>;
}
