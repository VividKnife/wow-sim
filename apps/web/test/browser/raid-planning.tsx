  ...
  return <main>
    ...
    <div className="workbench"><section aria-labelledby="roster-heading"><div className="section-heading"><h2 id="roster-heading">团队编成</h2><span>{plan.size / 5}队 × 5人</span></div>
      <div className="squads">{plan.squads.map((squad, index) => <article className={`squad ${squad.controller.kind === 'account' ? 'core' : ''}`} key={squad.id}>
        <div className="squad-title"><span className="squad-number">{String(index + 1).padStart(2, '0')}</span><div><h3>{squad.name}</h3><small>{squad.controller.kind === 'account' ? '核心队 · 逐人配装与天赋' : '公会队 · 整队训练与补给'}</small></div></div>
    ...
    <footer><strong>25人基线：2坦克＋5治疗＋18输出。特殊首领按机制调整。</strong><span>正式路线：单人首领闭环 → 公会养成 → 2–5人自愿合作。40人保留为规模对照。</span></footer>
  </main>;
}
