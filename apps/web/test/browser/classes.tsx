...
function Harness(){
 ...
 return <main className="game-shell" style={{paddingTop:24,paddingBottom:32}}><section className="panel"><div className="eyebrow">真实引擎 / 独立夹具</div><h1>九职业界面验证</h1><p>固定等级 60、10000 金，不连接 API 或用户存档。所有操作直接调用游戏引擎。</p><div className="action-row"><label>职业 <select aria-label="测试职业" value={classId} onChange={event=>selectClass(Number(event.target.value))}>{classOptions.map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label>种族 <select aria-label="测试种族" value={raceId} onChange={event=>selectRace(Number(event.target.value))}>{racesForClass(classId).map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><button onClick={()=>{let next=structuredClone(state),changed=true;while(changed){changed=false;for(const a of view(next).skills.filter((a:any)=>a.canTrain)){try{const learned=act(next,{type:'train',id:a.spellId},next.wallAt);next=learned;changed=true;}catch{}}}next.hp=(stats(next) as any).maxHp;next.mana=(stats(next) as any).maxMana;replace(next);}}>学习所有可训练技能</button><button onClick={()=>{const next=structuredClone(state),ids=new Set<number>();for(const id of next.learned){const sp=spells[id];for(let n=1;n<=8;n++)if(sp?.['Reagent'+n]>0)ids.add(sp['Reagent'+n]);}for(const id of ids)addItem(next,id,5);replace(next);}}>补充测试材料</button><button onClick={()=>{const next=structuredClone(state);startCombat(next,[299]);replace(next);setOpen(true);}}>开始测试战斗</button><button onClick={()=>replace(advance(state,state.wallAt+30000,{dungeonOnline:true}).state)}>推进 30 秒</button><button onClick={()=>setOpen(true)} disabled={!state.combat&&!state.lastCombat}>查看战斗</button><button onClick={()=>setShowParty(value=>!value)}>{showParty?'隐藏队伍':'查看队伍'}</button><button onClick={()=>replace(fixture(classId,raceId))}>重置当前职业</button></div></section><Character {...props}/>{showParty&&<Party {...props}/>}<Battle {...props} open={open} onOpenChange={setOpen}/>{error&&<p role="alert" className="error">{error}</p>}</main>;
}
