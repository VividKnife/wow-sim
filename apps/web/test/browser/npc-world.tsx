...
const storage='npc-world-preview-v3';
function initial(){
 ...
 s=act(s,{type:'npcVisit'},0);s=act(s,{type:'npcRecommend'},0);
...
function Harness(){
 ...
 const send=async(command:any)=>{try{const before=latest.current;let next=act(before,command,before.wallAt);if(command.type==='leaveDungeon')next.party=[];commit(next);setError('');return true;}catch(e){setError((e as Error).message);return false;}};
